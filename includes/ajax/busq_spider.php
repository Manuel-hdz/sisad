	...
	if(!$db) {
		...
		if(isset($_POST['queryString'])) {
			...
			if(strlen($queryString) >0) {			
				...
				$query = $db->query("SELECT ".$_GET['nomCampo']." FROM ".$_GET['nomTabla']." WHERE ".$_GET['nomCampo']." LIKE '%$queryString%' LIMIT 10");
				if($query) {
					...
					while ($result = $query ->fetch_object()) {
						...
						// Mostrar la lista en el siguiente formato $result->nombreColumna
	         			echo utf8_encode('<li onClick="fill(\''.$_GET['nomCajaTexto'].'\' , \''.$result->$_GET['nomCampo'].'\' , \''.$_GET['num'].'\');">'.$result->$_GET['nomCampo'].'</li>');
