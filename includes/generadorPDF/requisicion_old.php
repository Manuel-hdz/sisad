...
class PDF extends PDF_MySQL_Table{
	...
	$comentario = obtenerDato($base,"requisiciones", "observaciones", "id_requisicion", $num_req);
	//Connect to database 
	conecta('$base');
	...
	$estado = obtenerDato($base,"requisiciones","estado","id_requisicion",$num_req);
	if ($estado=="PEDIDO")
	...
	$a_solicita = obtenerDato($base,"requisiciones", "area_solicitante", "id_requisicion", $num_req);
	$f = obtenerDato($base,"requisiciones", "fecha_req", "id_requisicion", $num_req);
	$fecha = modFecha($f,2);
	$solicitante = obtenerDato($base,"requisiciones", "solicitante_req", "id_requisicion", $num_req);
	$elaboro = obtenerDato($base,"requisiciones", "elaborador_req", "id_requisicion", $num_req);
	if($elaboro=="")
		$elaboro="                                                                           ";
	$jt = obtenerDato($base,"requisiciones", "justificacion_tec", "id_requisicion", $num_req);
	...
	$pdf->Cell(60);
	$pdf->Cell(10,10,'REQUISICIÓN DE COMPRA',0,0);
	$pdf->Cell(43);
	...
	$pdf->Cell(92,5,'Descripción.','LTRB',0,'C',1);
	$pdf->Cell(58,5,'Aplicación.','LTRB',1,'C',1);
	...
	//Definir el tipo y tamaño de la letra para el Detalle de la Requisicion
	$pdf->SetFont('Arial','',9);
	$cant_renglones = 0;
	//Dibujar los registros del Detalle de la Requisición
	while($registro=mysql_fetch_array($rs)){
		...
		$descripcion = cortarCadena($registro['descripcion'],45);
		$aplicacion = cortarCadena($registro['aplicacion'],28);		
		...
		for($i=0;$i<$maxRenglones;$i++){//Dibjar el Renglon con los datos del Registro
			...
			/*				
			//Imprimir la Unidad de Media
			if(isset($unidad[$i]))							
				$pdf->Cell(20,5,$unidad[$i],'LR',0,'C',0);
			else
				$pdf->Cell(20,5,'','LR',0,'',0);
			*/
	...
	if ($comentario!=""){		
		...
		$datosComentario = cortarCadena($comentario,45);
		...
		$pdf->Cell(92,5,"NOTA: ",'LR',0,'L',0);
		//Colocar la columna correspondiente a la Aplicación
		$pdf->Cell(58,5,'','LR',1,'C',0);						
		...
		for($i=0;$i<$datosComentario['cantRenglones'];$i++){
			...
			if($i==0){
				...
				$pdf->SetFont('Arial','',9);
				$pdf->Cell(92,5,$datosComentario[$i],'LR',0,'L',0);							
			}
			else
				$pdf->Cell(92,5,$datosComentario[$i],'LR',0,'L',0);
	...
	/***********************************************JUSTIFICACION**************************************************/
	/**************************************************************************************************************/		
	//Agregar el titulo de la Justificación al renglon en la base de la tabla
	$pdf->SetFont('Arial','B',12);//Formato del Texto del titulo de la Justificacion
	$pdf->Cell(190,6,'JUSTIFICACIÓN TÉCNICA CON REFERENCIAS y/o ANTECEDENTES','LTRB',1,'C',1);
	//Aqui se escribre la justificación tecnica obtenida de la BD
	...
	$pdf->Cell(95,5,'Almacén',0,1,'C');
	$pdf->Cell(95,5,'  ',0,1,'C');
	$pdf->Cell(95,5,'______________________________',0,0,'C');
	$pdf->Cell(95,5,'______________________________',0,1,'C');
	$pdf->Cell(95,5,'  ',0,1,'C');
	$pdf->Cell(95,5,'  ',0,1,'C');
	$pdf->SetFont('Arial','U',10);
	$pdf->Cell(95,5,'Elaboró',0,0,'C');
	$pdf->Cell(95,5,'Solicitó',0,1,'C');
	$pdf->Cell(95,5,'  ',0,1,'C');
		...
		if(is_dir($ruta)){
			...
			if($gestor=opendir($ruta)){
				...
				while(false!==($nomFoto=readdir($gestor))){				
					...
					//La extensión para verificar que sea JPG y que se pueda  acceder a ellas 
					if ($nomFoto != "." && $nomFoto != ".."&&strtoupper(substr($nomFoto,-3))=='JPG') {
						...
						if(fmod($contador,3)==0){
							...
							$material=obtenerDato("bd_almacen","materiales", "nom_material", "id_material", $clave);
	...
	$pdf->SetTitle("REQUISICIÓN ".$num_req);
