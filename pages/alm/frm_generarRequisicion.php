...
<Sistema de Gestion Empresarial, Produccion y Operacion html xmlns="http://www.w3.org/1999/xhtml">
	...
	if(!verificarPermiso($usr_reg,$_SERVER['PHP_SELF'])){		
		...
		if ( ($usr_reg != 'sistemas') && ($usr_reg != 'Almacen') && ($usr_reg != 'AuxAlmacenUno')&& ($usr_reg != 'AuxProLilia') && ($usr_reg != 'AuxCom3')  && ($usr_reg != 'AuxJuan') ) { 
			$busq_material = 'busq_spider_material_req.js';
		} else {
			$busq_material = 'busq_spider_material_req_almacen.js';
		}
?>
	...
	<head>
		...
		<script type="text/javascript" src="../../includes/ajax/<?php echo $busq_material; ?>"></script>
	...
	if(!isset($_POST['txt_areaSolicitante'])){
		...
		$solicitante=obtenerDato("bd_recursos","organigrama","empleados_rfc_empleado","departamento",$area);
		//Con el RFC, traer el nombre completo del encargado del depto
		$solicitante=obtenerNombreEmpleado($solicitante);
	...
	?>
			...
			<form name="frm_generarRequisicion" method="post" action="frm_generarRequisicion.php">
				<table border="0" cellpadding="5" cellspacing="5" class="tabla_frm">
					<tr>
						...
						<td>
							<input type="text" name="cmb_material" id="cmb_material" class="caja_de_texto" size="30"
								onkeyup="lookup(this,'1');" value="" maxlength="60" autocomplete="off" required="required" />
							<div id="res-spider">
						...
						<td align="left">
							<input type="text" name="txt_clave" id="txt_clave" size="10" maxlength="10" readonly="true"
								style="text-align:center;" required="required" />
						</td>
						<td align="right">Existencia</td>
						<td align="left">
							<input type="text" name="txt_existencia" id="txt_existencia" size="3" maxlength="5" readonly="true"
								style="text-align:center;" required="required" />
						</td>
					</tr>
					<tr>
						...
						<td align="left">
							<input type="text" name="txt_unidadMedida" id="txt_unidadMedida" size="5" maxlength="10"
								style="text-align:center;" required="required" />
						...
						<td>
							<input name="txt_cantReq" type="text" class="caja_de_texto" id="txt_cantReq" size="5" maxlength="10"
								onkeypress="return permite(event,'num',2);" required="required" autocomplete="off" />
						</td>
						<td>
				...
				if($datos = mysql_fetch_array($rs)){?>
							<select name="cmb_con_cos" id="cmb_con_cos" class="combo_box"
								onchange="cargarCuentas(this.value,'cmb_cuenta')" required="required">
				...
				?>
					...
					-->
								<select name="cmb_cuenta" id="cmb_cuenta" class="combo_box"
									onchange="cargarSubCuentas(cmb_con_cos.value,cmb_cuenta.value,cmb_subcuenta.name);"
									required="required">
						...
						<td>
							<span id="datosSubCuenta">
								<select name="cmb_subcuenta" id="cmb_subcuenta" class="combo_box"
									onchange="hdn_subcuenta.value = this.value" required="required">
		...
		<fieldset id="datos-gral" class="borde_seccion">
			...
			<br>
			<form onsubmit="return valFormInformacionRequisicion(this);" name="frm_InformacionRequisicion" method="post"
				action="frm_generarRequisicion.php">
				<table width="100%" border="0" cellpadding="5" class="tabla_frm">
					<tr>
						...
						<td>
							<textarea name="txa_justificacionReq" maxlength="300" onkeyup="return ismaxlength(this)" cols="30"
								rows="2" class="caja_de_texto" id="txa_justificacionReq" onkeypress="return permite(event,'num_car');"
						...
						<td>
							<input name="txt_elaboradorReq" type="text" id="txt_elaboradorReq"
								onkeypress="return permite(event,'num_car');" readonly="readonly" size="40" maxlength="60"
								value="<?php echo $elaborador;?>" />
					...
					?>
								...
								<option <?php if($cmb_prioridad=="URGENTE") echo "selected='selected'"; ?> value="URGENTE">URGENTE
								</option>
							</select>
						</td>
					</tr>
					<tr>
						...
						<td>
							<input name="txt_areaSolicitante" type="text" id="txt_areaSolicitante"
								onkeypress="return permite(event,'num_car');" size="25" maxlength="45" value="ALMAC&Eacute;N"
								required="required" />
						</td>
						<td>
						...
						<td>
							<input name="txt_solicitanteReq" type="text" id="txt_solicitanteReq"
								onkeypress="return permite(event,'num_car');" size="34" maxlength="60"
								value="<?php echo $solicitante;?>" required="required" />
						</td>
					</tr>
					<tr>
						<td>
					...
					<tr>
						<td colspan="6" align="center">
							<input type="hidden" name="hdn_materialAgregado" id="hdn_materialAgregado" <?php 
				if(isset($_POST['txt_cantReq']) || isset($_POST['txt_cantReq2']) || (isset($_SESSION['datosRequisicion']) && count($_SESSION['datosRequisicion'])>0) ) 
					echo "value='si'"; 
				else 
					echo "value='no'"; 
				?> />
							<input name="sbt_generar" type="submit" class="botones" id="sbt_generar" value="Generar"
								onMouseOver="window.status='';return true" title="Generar Requisici&oacute;n" />
							<?php
				...
				if (isset($_SESSION["id_requisicion"])){
					...
					$id_req = $_SESSION["id_requisicion"];
							...
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							<input name="btn_comentario" type="button" class="botones" value="Comentario"
								title="Ingresar Comentario a la Requisici&oacute;n"
		...
		if(isset($_POST["btn_agregarOtro"])){
			...
			<script type="text/javascript" language="javascript">
				setTimeout("alert('El Material ya fue Agregado a la Requisici�n');", 500);
			</script>
			<?php
				}
			} else {
