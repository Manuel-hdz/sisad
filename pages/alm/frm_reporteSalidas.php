...
<head>
    ...
    <style type="text/css">
		<!--
		#form-datos-salida2 {position:absolute;	left:30px; top:190px; width:400px; height:207px; z-index:13; }
		#form-datos-salida-cat {position:absolute;	left:30px; top:430px; width:910px; height:250px; z-index:13; }
		...
		#calendario_repInicio2 { position:absolute; left:275px; top:228px; width:29px; height:24px; z-index:14; }
		#calendario_repCierre2 { position:absolute; left:275px; top:265px; width:30px; height:26px; z-index:15; }
		#calendario_repInicioCat { position:absolute; left:255px; top:455px; width:29px; height:24px; z-index:14; }
		#calendario_repCierreCat { position:absolute; left:761px; top:455px; width:30px; height:26px; z-index:15; }
...
<body>
	...
	if(!isset($_POST['txt_fechaInicio']) && !isset($_POST['fecha_ini'])){?>	
		...
		<table border="0" align="center" cellpadding="5" width="100%" cellspacing="5" class="tabla_frm">
				...
				<td>
					...
					?>
						<select name="cmb_cc" id="cmb_cc" size="1" class="combo_box" required="required">
							...
							<option value="EPP">EPP</option>--><?php 
							do{
			...
			<input type="hidden" name="cmb_cuenta" id="cmb_cuenta" value=""/>
			<tr>
				<td>
				  <div align="center">
		...
		<fieldset id="form-datos-salida-cat" class="borde_seccion">
			<legend class="titulo_etiqueta">Seleccionar Filtros</legend>
			<form name="frm_datosReporteSalidasCat" action="frm_reporteSalidas.php" method="post" onsubmit="return valFormFechas(this);">
				<table border="0" align="center" cellpadding="5" width="100%" cellspacing="5" class="tabla_frm">
					<tr>
						<td>
					...
					<tr>
						...
						<td>
							<?php
							$conn = conecta("bd_recursos");
							$rs = mysql_query("SELECT * FROM  control_costos WHERE  habilitado LIKE  'SI' ORDER BY descripcion");
							if($rs){
								$dato = mysql_fetch_array($rs);
								?>
								<select name="cmb_cc" id="cmb_cc" class="combo_box"/>
									...
									do{
										?>
										<option value="<?php echo $dato["id_control_costos"]; ?>"><?php echo $dato["descripcion"]; ?></option>
										<?php
						...
						<td>
							<?php
							$conn = conecta("bd_recursos");
							$rs = mysql_query("SELECT * FROM  cuentas WHERE  habilitado LIKE  'SI' ORDER BY descripcion");
							if($rs){
								$dato = mysql_fetch_array($rs);
								?>
								<select name="cmb_cuenta" id="cmb_cuenta" class="combo_box"/>
									...
									do{
										?>
										<option value="<?php echo $dato["id_cuentas"]; ?>"><?php echo $dato["descripcion"]; ?></option>
										<?php
									}while($dato = mysql_fetch_array($rs));
									?>
								</select>
					...
					<tr>
						...
						<td>
							...
							$rs = mysql_query("SELECT * FROM  categorias_mat WHERE  habilitado LIKE  'SI' ORDER BY descripcion");
							if($rs){
								$dato = mysql_fetch_array($rs);
								?>
								<select name="cmb_cat" id="cmb_cat" class="combo_box"/>
									<option value="TODAS">TODAS</option>
									<?php
									do{
										?>
										<option value="<?php echo $dato["id_categoria"]; ?>"><?php echo strtoupper($dato["descripcion"]); ?></option>
										<?php
									}while($dato = mysql_fetch_array($rs));
									?>
								</select>
						...
						<td>
							...
							$rs = mysql_query("SELECT * FROM  `equipos` WHERE  `estado` LIKE  'ACTIVO' AND  `disponibilidad` LIKE  'ACTIVO' ORDER BY id_equipo");
							if($rs){
								$dato = mysql_fetch_array($rs);
								?>
								<select name="cmb_equipo" id="cmb_equipo" class="combo_box"/>
									...
									do{
										?>
										<option value="<?php echo $dato["id_equipo"]; ?>"><?php echo $dato["id_equipo"]; ?></option>
										<?php
									}while($dato = mysql_fetch_array($rs));
									?>
								</select>
					...
					<tr>
						<td>
							<div align="right">Dolar</div>
						</td>
						<td>
							$<input name="txt_dolar" type="text" class="caja_de_num" id="txt_dolar" autocomplete="off" onchange="formatCurrency(value,'txt_dolar');" onkeypress="return permite(event,'num');" size="15" maxlength="20" required="required"/>
						</td>
						<td>
							<div align="right">Euro</div>
						</td>
						<td>
							$<input name="txt_euro" type="text" class="caja_de_num" id="txt_euro" autocomplete="off" onchange="formatCurrency(value,'txt_euro');" onkeypress="return permite(event,'num');" size="15" maxlength="20" required="required"/>
						</td>
					</tr>
					<tr>
						<td colspan="4" align="center">
							<input name="sbt_verReporteCat" type="submit" class="botones" value="Ver Reporte" onMouseOver="window.status='';return true" title="Ver Salidas Registradas de Material por Categorias"  />
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		...
		<div id="calendario_repInicioCat">
			<input name="calendario_iniRep" type="image" id="calendario_iniRep" onclick="displayCalendar(document.frm_datosReporteSalidasCat.txt_fechaInicio,'dd/mm/yyyy',this)" 
			onmouseover="window.status='';return true" src="../../images/calendar.png" align="absbottom" width="25" height="25" border="0" />
		</div>
		
		<div id="calendario_repCierreCat">
			<input name="calendario_cieRep" type="image" id="calendario_cieRep" onclick="displayCalendar(document.frm_datosReporteSalidasCat.txt_fechaCierre,'dd/mm/yyyy',this)" 
			onmouseover="window.status='';return true" src="../../images/calendar.png" align="absbottom" width="25" height="25" border="0" />
		</div>
	...
	<?php
		...
		if (isset($_POST["sbt_verReporte"])){
			...
			$res=mostrarSalidas2($txt_fechaInicio,$txt_fechaCierre);
			echo "</div>";
			?>
			...
			<table width="50%" cellpadding="12">
				<tr><td colspan="2">
					...
					?>
							...
							<input type="hidden" name="hdn_tipoReporte" id="hdn_tipoReporte" value="salidasDetalle"/>
							<input type="submit" onclick="document.getElementById('hdn_tipoReporte').value='salidasDetalle'" value="Exportar a Excel" 
							name="sbt_exportar" id="sbt_exportar" class="botones" title="Exportar el Reporte a Excel" onmouseover="window.status='';return true;"/>
							&nbsp;&nbsp;
							<input type="submit" onclick="document.getElementById('hdn_tipoReporte').value='generarReporteSalidas'" value="Generar Reporte Salidas" style="width:170px";
							name="sbt_gen_salidas" id="sbt_gen_salidas" class="botones" title="Generar Reporte de Salidas" onmouseover="window.status='';return true;"/>
							&nbsp;&nbsp;
					<?php
						}
					?>
			...
			<?php
		} else if(isset($_POST["sbt_verReporteCat"])) {
			$res = mostrarSalidasCat($txt_fechaInicio,$txt_fechaCierre);
			?>
			</div>
			<div id="btns-regpdf" align="center">
						...
						<td>
							<form method="post" action="guardar_reporte.php">
								...
								?>
									...
									<input type="hidden" name="hdn_fechaF" id="hdn_fechaF" value="<?php echo $txt_fechaCierre;?>"/>
									<input type="hidden" name="hdn_cc" id="hdn_cc" value="<?php echo $cmb_cc;?>"/>
									<input type="hidden" name="hdn_cuenta" id="hdn_cuenta" value="<?php echo $cmb_cuenta;?>"/>
									<input type="hidden" name="hdn_cat" id="hdn_cat" value="<?php echo $cmb_cat;?>"/>
									<input type="hidden" name="hdn_equipo" id="hdn_equipo" value="<?php echo $cmb_equipo;?>"/>
									<input type="hidden" name="hdn_dolar" id="hdn_dolar" value="<?php echo $txt_dolar;?>"/>
									<input type="hidden" name="hdn_euro" id="hdn_euro" value="<?php echo $txt_euro;?>"/>
									
									<input type="hidden" name="hdn_consulta"/>
									<input type="hidden" name="hdn_tipoReporte" id="hdn_tipoReporte" value="salidasDetalleCat"/>
									<input type="submit" onclick="document.getElementById('hdn_tipoReporte').value='salidasDetalleCat'" value="Exportar a Excel" 
									name="sbt_exportar" id="sbt_exportar" class="botones" title="Exportar el Reporte a Excel" onmouseover="window.status='';return true;"/>
									&nbsp;&nbsp;
									<input type="submit" onclick="document.getElementById('hdn_tipoReporte').value='ReporteSalidasCat'" value="Generar Reporte Salidas" style="width:170px";
									name="sbt_gen_salidas" id="sbt_gen_salidas" class="botones" title="Generar Reporte de Salidas" onmouseover="window.status='';return true;"/>
									&nbsp;&nbsp;
								<?php
								}
								?>
								<input name="btn_regresar" type="button" value="Regresar" class="botones" title="Seleccionar otro filtros" onclick="location.href='frm_reporteSalidas.php'" />
							</form>
						</td>
					</tr>
				</table>
