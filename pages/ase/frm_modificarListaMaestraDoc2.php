...
<?php
    ...
    <div class="titulo_barra" id="titulo-registrar">Modificar Lista Maestra de Registros Calidad </div><?php		
		//Conectar a la BD de Aseguramiento
		$conn = conecta("bd_aseguramiento");
		if(!isset($_POST['sbt_guardar'])){
			//Fragmentamos el rdb
			$radioSel=explode('/',$_POST['rdb_id']);
			$idManual=$radioSel[0];
			$idProc=$radioSel[1];
			//Crear sentencia SQL
			$sql_stm ="SELECT id_manual, nombre, manual_calidad.no_rev AS revision_manual,manual_calidad.entrada_vigor, id_clausula, titulo_clausula 
			FROM (((manual_calidad JOIN catalogo_clausulas ON id_manual=catalogo_clausulas.manual_calidad_id_manual) JOIN 
					   	catalogo_procedimientos ON id_manual=catalogo_procedimientos.manual_calidad_id_manual)JOIN lista_maestra_documentos ON 
						id_manual=lista_maestra_documentos.manual_calidad_id_manual) WHERE id_manual = '$idManual' AND id_procedimiento='$idProc'";
		...
		/*Verificamos que exista el boton guardar; de ser asi asignar a las variables los valores correspondientes. Considerando los siguientes casos*/		
		/*Verificamos que exista el boton guardar; de ser asi asignar a las variables los valores correspondientes. Considerando los siguientes casos*/		
		$claveManual = $datos['id_manual'];
		$tituloManual = $datos['nombre'];
		$noRevManu = $datos['revision_manual'];
		$claveClausula = $datos['id_clausula'];
		$tituloClausula = $datos['titulo_clausula'];
		$fecha = modFecha($datos['entrada_vigor'],1);
	}
	if(isset($_POST['sbt_guardar'])){
		$claveManual=$_POST["txt_claveManual"];
		$tituloManual=$_POST["txa_tituloManual"];
	...
	?>
		<form  onsubmit="return valFormModForm(this);"action="frm_modificarListaMaestraDoc2.php" method="post"name="frm_modificarRegistro" id="frm_modificarRegistro">
    		...
    		<tr>
        		<td width="131" height="31"><div align="right">*Clave Manual </div></td>
         		<td width="212">
					...
					value="<?php echo $claveManual; ?>" <?php if(isset($_SESSION['bandera'])){?> readonly="readonly" <?php }?>/>
				</td>
    	...
    	<table width="764" height="134"  cellpadding="5" cellspacing="5" class="tabla_frm">
    		<tr>
        		<td width="131" height="31"><div align="right">*Clave Clausula </div></td>
          		<td width="212">
					...
					<?php if(isset($_SESSION['bandera'])){?>onblur="return verificarClausula(this,'<?php echo $_SESSION['bandera'];?>');"<?php }?>/>
          			<span id='errorClausula' class="msj_error">Clave Duplicada</span>				</td>
        		...
        		<td width="197">	
					...
					cols="30"onkeypress="return permite(event,'num_car', 0);"><?php echo $tituloClausula; ?></textarea>				</td>
        	</tr>
        	<tr>
        	...
        	<tr>
        		<td colspan="4">
					<div align="center">
		   				<input name="btn_modFormato" type="button" class="botones_largos" id= "btn_regFormato" value="Modificar Formatos" title="Modificar Formatos" 
						onmouseover="window.status='';return true" onclick="envioDatosGetMod();" />
		   				&nbsp;&nbsp;&nbsp;
            			<input name="sbt_guardar" type="submit" class="botones" id= "sbt_guardar" value="Modificar" 
						title="Guardar Registro De Lista Maestra" onmouseover="window.status='';return true" />
            			&nbsp;&nbsp;&nbsp;
            			<input name="rst_limpiar" type="reset" class="botones"  value="Restablecer" title="Limpiar Formulario" 
						onmouseover="window.status='';return true"/>
            			&nbsp;&nbsp;&nbsp;
            			<input name="btn_cancelar" type="button" class="botones" value="Cancelar" title="Regresar al Men&uacute; Repositorio"
						onmouseover="window.status='';return true"  onclick="confirmarSalida('frm_modificarListaMaestraDoc.php')" />
            			<input type="hidden" name="hdn_claveValida" id="hdn_claveValida" value="si"/>
						<input type="hidden" name="hdn_procedimiento" id="hdn_procedimiento" value="<?php echo $_POST['hdn_procedimiento'];?>"/>
						<input type="hidden" name="hdn_clausula" id="hdn_clausula" value="<?php echo  $_POST['hdn_clausula'];?>"/>
          			</div>				
				</td>
        	</tr>
      	</table>
		...
		<div id="calendario">
			<input name="calendario" type="image" id="calendario2" onclick="displayCalendar (document.frm_modificarRegistro.txt_fecha,'dd/mm/yyyy',this)" 
