	...
	if(!$db) {
		...
		if(isset($_POST['queryString'])) {
			...
			if(strlen($queryString) >0) {
				...
				$query = $db->query("SELECT rfc_empleado,puesto,CONCAT(nombre,' ',ape_pat,' ',ape_mat) AS nombre FROM empleados WHERE CONCAT(nombre,' ',ape_pat,' ',ape_mat) LIKE '%$queryString%' LIMIT 10");
				if($query) {
					...
					while($result = $query ->fetch_object()){// Establece los resultados a modo de lista.
						...
						//echo utf8_encode('<li title='.$result->puesto.' onClick="fill(\''.$_GET['nomCajaTexto'].'\' , \''.$result->nombre.'\' , \''.$_GET['num'].'\');">'.$result->nombre.'</li>');
