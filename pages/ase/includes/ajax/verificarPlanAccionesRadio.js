/**
  ...
  */
	...
	function verificaPlanAccionesRadio(idPA){
		...
		var url = "includes/ajax/verificarPlanAccionesRadio.php?idPa="+idPA;
		...
		iniciarPeticion(url, "GET", procesarRegistrosRadio);	
	...
	function procesarRegistrosRadio(){
		//Verificar que la peticion HTTP se haya realizado correctamente
 		if(pet_http_vd.readyState==READY_STATE_COMPLETE){
			if(pet_http_vd.status==200){
				...
				if (existe=="false"){		
					alert("El Registro no ha Sido Complementado por el Departamento");
				}
