<?php
				...
				<?php if($noReg!=0){
				 ...
				 <?php }
					   elseif($noReg>1){?>
							...
							title="Descargar Documento<?php echo $arrArch['nombre'];?>" 
							onClick="javascript:window.open('verArchivosExternos.php?idAlerta=<?php echo $datos['id_alerta'];?>&depto=<?php echo $depto;?>',
	...
	function mostrarArchivos($idAlerta, $depto){
		...
		//Comprobar cual es el departamento que viene en la funcion contenida en verArchivosExternos
		if($depto=="seg"){
			//Realizar la conexion a la BD de Seguridad
			$conn = conecta("bd_seguridad");
		}
