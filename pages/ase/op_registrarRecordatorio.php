<?php
	...
	function obtenerIdReg(){
		...
		$stm_sql = "SELECT MAX(id_alerta) AS cant FROM alertas_generales WHERE id_alerta LIKE 'ALR$mes$anio%'";
			...
			do{	
				echo "	<tr>
							<td class='nombres_filas' align='center'>
								<input type='checkbox' id='ckb_arch$cont' name='ckb_arch$cont' value='$datos[id_documento]' onclick='quitarArch(this);'/>
							</td>				
	...
	function guardarRegistro(){
		...
		if ($band==1){
			...
			if($dptos!=""){
				...
				registrarOperacion("bd_aseguramiento",$idRecordatorio,"RegRecordatorio",$_SESSION['usr_reg']);
