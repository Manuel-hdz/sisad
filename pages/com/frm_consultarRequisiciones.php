...
<?php
	...
	//Enviar a la pagina de acceso negado
		...
		include_once ("op_consultarRequisiciones.php");?>
...
<head>
    ...
    <style type="text/css">
		...
		#botones{position:absolute;left:30px;top:680px;width:940px;height:37px;z-index:13;}
		#tabla-resultados{position:absolute; left:30px; top:191px; width:930px; height:400px; z-index:15; overflow: scroll;}
		#detalle_Req{position:absolute;overflow:auto;left:30px;top:400px;width:950px;height:235px;z-index:13;}
		...
		#tabla-estado { position:absolute; left:560px; top:190px; width:420px; height:170px; z-index:12; }
...
<body>
	...
	if (isset($_POST["btn_guardarEstado"])){
		...
		$estadoReq = $_POST["cmb_estado"];
		//Llamamos a la funcion que almacena el comentario con la clave de requisicion, el comentario y el departamento de trabajo actual
		guardarEstado($cveReq,$comentario,$departamento,$base,$estadoReq);
	}
	...
	$ctrl=0;
	//Esta comparación se realiza para verificar que el departamento este declarado
	if (isset($_GET["depto"]) || isset($_GET["nomBD"])){
/*		echo "<meta http-equiv='refresh' content='10;url=menu_requisiciones.php'>";
		...
		if (isset($_POST["btn_revisar"]) || isset($_POST["btn_guardarC"]) || isset($_POST["btn_guardarEstado"]) || isset($_POST["origen"]) || isset($_GET["nomBD"])){
			if(!isset($_GET["nomBD"])){
			...
			echo "<form method='post' name='frm_detallesRequisicion' id='frm_detallesRequisicion'>";
			echo "<fieldset id='tabla-requisiciones' class='borde_seccion'>
				<legend class='titulo_etiqueta'>Requisici&oacute;n ".$cve_req."</legend>";
			
			//Se ejecuta la funcion que muestra el detalle de las requisiciones del departamento
			if(isset($_GET["nomBD"])){
				$departamento = $_GET["depart"];
				$base = $_GET["nomBD"];
				$cve_req = $_GET["clave"];
			}
			$estado=mostrarRequisicionDetalle($departamento,$base,$cve_req);
			
			echo "</fieldset>";?>

			<div id='botones' align="center">
				<input type="hidden" name="rdb_req" value="<?php echo $cve_req?>"/>
				<input name="btn_guardarEstado" type="submit" id="btn_guardarEstado" class="botones" value="Guardar Estado"
				onMouseOver="window.status='';return true" title="Guardar Estado para la Requisici&oacute;n <?php echo $cve_req;?>" <?php if($estado == "ENTREGADA") echo "disabled='disabled'"; ?>/>
				&nbsp;&nbsp;&nbsp;&nbsp;
				<!--
				<input name="btn_guardarC" type="submit" id="btn_guardarC" class="botones" value="COTIZAR PARA AUTORIZACI&Oacute;N" style="width:250px"
				onMouseOver="window.status='';return true" title="Guardar Comentario y Estado para la Requisici&oacute;n <?php echo $cve_req;?>" <?php if($estado == "ENTREGADA") echo "disabled='disabled'"; ?>/>
				&nbsp;&nbsp;&nbsp;&nbsp;
				-->
				...
				?>
				<input name="sbt_regresar" type="button" value="Regresar" class="botones" title="Regresar a las Requisiciones Publicadas" onMouseOver="window.status='';return true" onclick="location.href='menu_requisiciones.php'"/>
				&nbsp;&nbsp;&nbsp;&nbsp;
				<?php
				}
				if(!isset($_GET["nomBD"])){
				...
				id="sbt_generar" value="Registrar Pedido" onMouseOver="window.status='';return true"<?php
					...
					if($estado=="CANCELADA" || $estado=="NO AUTORIZADA"){
						echo " title='No se Puede Registrar un Pedido de una Requisici&oacute;n Cancelada o No Autorizada'";
					}
					else if($estado=="PEDIDO" || $estado=="ENTREGADA"){
						echo " title='Ya se Registr&oacute; un Pedido de La Requisici&oacute;n $rdb_req'";
					}
					else{
			...
			<?php
				echo "<div id='detalle_Req' class='borde_seccion2'>";
				//Se ejecuta la funcion que muestra el detalle de las requisiciones del departamento
				dibujarDetalle($cve_req,$departamento,$base);
				echo "</div>";
			?>
			</form><?php
			...
			if (isset($_POST["sbt_consultarReq"]) || $ctrl==1 || isset($_POST["sbt_regresar"])){
				//Definimos el FORM que actuara para los botontes declarados mas delante
				echo "<form name='frm_consultarRequisiciones' method='post' action=''>";
				...
				if ($ctrl==1) {
					...
					<input type='submit' name='btn_registrar' id='btn_registrar' class='botones' on value='Guardar Estado'
					title="Guardar Estado de la Requisici&oacute;n" onMouseOver="window.status='';return true"/>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				...
				<input type='button' onclick="location.href='frm_consultarRequisiciones.php?depto=<?php echo $_GET["depto"];?>'" class='botones' value='Regresar'
				...
				<form name="frm_buscarRequisiciones" action="frm_consultarRequisiciones.php?depto=<?php echo $_GET["depto"];?>&bus=fecha" method="post" onsubmit="return valFormReqFecha(this);">
				...
				<form name="frm_buscarRequisiciones2" action="frm_consultarRequisiciones.php?depto=<?php echo $_GET["depto"];?>&bus=combo" method="post" onsubmit="return valFormReqEdo(this);">
					...
					<tr>
						...
						<td>
							<select name="cmb_estadoBuscar" class="combo_box">
								...
								<option value="4">COTIZANDO</option>
								<option value="5">EN PROCESO</option>
								<option value="6">EN TRANSITO</option>
								<option value="7">ENTREGADA</option>
								<option value="8">AUTORIZADA</option>
								<option value="9">NO AUTORIZADA</option>
