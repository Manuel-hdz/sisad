...
<head>
	...
	<style type="text/css">
		...
		#tabla-pedido {position:absolute;left:30px;top:190px;width:900px;height:308px;z-index:16;}		
...
<body>
	...
	if (!isset($_POST["hdn_numero"])){
		...
		<legend class="titulo_etiqueta">Registrar Detalles del Pedido</legend>
	 	<form id="frm_detallePedido" name="frm_detallePedido" method="post" onsubmit="return valFormRegistrarDetallePedido(this)" action="">
		<table width="100%" cellpadding="5" class="tabla_frm">
    		<tr>
       		  <td width="15%"><div align="right">Pedido</div></td>
	        	<td width="30%">
			  <input name="txt_pedido" id="txt_pedido" type="text" class="caja_de_texto" readonly="readonly" size="10" maxlength="10"
                    value="<?php echo $pedido;?>"/></td>
	          <td width="15%"><div align="right">*Descripcion</div></td>
    		    <td width="40%">
					...
					<span id="datosDescripcion" onchange="cargarDetalles(txa_descripcion.value,txt_requisicion.value,'cmb_unidad',txt_cantidad.id)">
			...
			<tr>
    			<td><div align="right">*Requisicion</div></td>
	    	    <td>
				<?php
				...
				<?php 
					...
					<span id="datosRequisicion">
						<select name="txt_requisicion" id="txt_requisicion" class="combo_box" onchange="cargarDetalleReq(this.value,'txa_descripcion',txt_partida.value,'')" tabindex="2">
        		...
        		<td>
                   	<input name="txt_cantidad" id="txt_cantidad" type="text"  onkeypress="return permite(event,'num', 2)" class="caja_de_texto"
                   	size="5" maxlength="10" onchange="formatCurrency(value*txt_preciouni.value.replace(/,/g,''),'txt_importe');" tabindex="3" 
					value="1" ondblclick="this.value='';" />
                </td>
			</tr>
			<tr>
				<td><div align="right">Partida</div></td>
		        <td>
                   	<input name="txt_partida" id="txt_partida" type="text" class="caja_de_num" value="<?php echo $cont;?>" readonly="true"
                   	size="1" maxlength="3" />
                </td>
			...
			<tr>
				...
				<td><?php 
					...
					if($unidades=mysql_fetch_array($rs_unidades)){?>
						<select name="cmb_unidad" id="cmb_unidad" class="combo_box" onchange="agregarNvaUnidad(this)" tabindex="5">
				...
				<td>$<input name="txt_importe" type="text" class="caja_de_texto" id="txt_importe" size="15" maxlength="20" readonly="true" /></td>
			</tr>
			<tr>
				...
				<td><?php 
					...
					if($equipos=mysql_fetch_array($rs_equipos)){?>
						<select name="cmb_equipos" id="cmb_equipos" class="combo_box" tabindex="6" onchange="agregarNvoEquipo(this);">
				...
				if(!isset($_POST['txt_nomProveedor'])){?>				
					<td>
						...
						value="" size="50" maxlength="80" onkeypress="return permite(event,'num_car', 0);" tabindex="7"/>
					...
					<td>
						<input type="text" name="txt_nomProveedor" id="txt_nomProveedor" class="caja_de_texto" readonly="readonly" 
						value="<?php echo $_POST['txt_nomProveedor']; ?>" size="50" maxlength="80" />
					</td><?php
				}?>
			</tr>
			<tr>
				...
				<td>
					<input type="text" name="txt_descto" id="txt_descto" class="caja_de_num" value="<?php echo number_format($descto,2,".",",");?>" size="6" maxlength="6" onkeypress="return permite(event,'num_car', 0);" 
					onchange="validarDescto(this);formatCurrency(value.replace(/,/g,''),'txt_descto');"<?php if(isset($_POST["btn_registrar"])) echo " readonly='readonly'";?>/>%
				</td>
			</tr>
			<tr><td colspan="4"><strong>*Los campos marcados con asterisco son <u>obligatorios</u></strong></td></tr>
      		<tr>
				<td colspan="4" align="center">
                    ...
                    <input name="btn_registrar" type="submit" class="botones_largos" id="btn_registrar" value="Registrar Detalle Pedido"
                    onmouseover="window.status='';return true;" title="Registrar Detalle del Pedido" tabindex="7" />
                    &nbsp;
					<?php
					if (isset($_SESSION["detallespedido"])){?>
						<input name="btn_finalizar" type="button" class="botones" value="Finalizar" onmouseover="window.status='';return true;"
                    	onclick="verIVA2();" title="Registrar Informaci&oacute;n Complementaria" tabindex="8" />
						&nbsp;<?php
					}?>
					<input name="rst_limpiar" type="reset" class="botones" value="Limpiar" title="Limpiar Formulario" 
					tabindex="9"<?php if(!isset($_POST["btn_registrar"])) echo " onclick='txt_descto.readOnly=false;'";?>/>&nbsp;
    	    		<input name="btn_cancelar" type="button" class="botones" id="boton-cancelar" value="Cancelar" title="Regresar al Men&uacute; de Pedidos"
	...
	//Si hdn_numero estan declarado, se llego aqui desde una requisicion, mandar llamar la funcion que muestra los datos para completarse
		...
		if($_POST["hdn_estado"]!="PEDIDO"){?>		
			<form name='frm_pedido_requisicion' onSubmit="return valFormDetallesPedido1(this);" method='post' action='frm_registrarPedido2.php'>
				...
				<div id='botones' align='center'>			
					...
					<input type="hidden" name="hdn_iva" id="hdn_iva"/>
					<input name="btn_continuar" type="submit" class="botones" value="Continuar" onmouseover="window.status='';return true;" title="Registrar Pedido"/>
					&nbsp;
					<input name="rst_limpiar" type="reset" class="botones" value="Limpiar" title="Limpiar Los Campos" onclick="restablecerCamposPedidoRead(cant_ckbs);"/>&nbsp;<?php 
					if(isset($_GET["origen"])){
						...
						<input name="btn_cancelar" type="button" class="botones" value="Regresar" title="Cancelar Registro del Pedido"
						...
						<input name="btn_cancelar" type="button" class="botones" value="Cancelar" title="Cancelar Registro del Pedido"
			...
			<script type='text/javascript' language='javascript'>
				setTimeout("alert('El Pedido de la Requisición seleccionada ya se Registró')",0);
			</script><?php
