...
?>
	...
	if(!isset($_POST["btn_registrarDetalle"])){?>
		<script type="text/javascript" language="javascript">
			...
			function convertirToNum(cadena){
				var valor = "";
			...
			function calcTotalPed(){
				var subtotal = parseFloat(convertirToNum(document.getElementById("txt_subtotal").value));
				var iva = parseFloat(document.getElementById("txt_iva").value);
				var total = subtotal + iva;

				formatCurrency(total,'txt_total');
			}
		</script><?php 
	}?>
	...
	if(!isset($_POST["btn_registrarDetalle"])){
		...
		//Si esta definido el boton 'btn_continuar' el pedido es Generado a partir de una Requisici�n
		if (isset($_POST["btn_continuar"])){
			$requisicion = $_POST["hdn_requisicion"];
			$base_datos = $_POST["hdn_bd"];
		...
		<fieldset class="borde_seccion" id="tabla-registro" name="tabla-registro">
			...
			<form onSubmit="return valFormRegistrarPedido(this);" name="frm_registrarPedido" id="frm_registrarPedido"
				method="post" action=""><?php
		if (isset($_POST["btn_continuar"]))
			//Colocar las cantidades de cada partida y el equipo asociado cuando el pedido se genera a partir de una requisici�n
			obtenerPrecioUnitReq();?>

				<input type='hidden' id='hdn_numero' name='hdn_numero' value='<?php echo $requisicion; ?>'>
				<input type='hidden' id='hdn_bd' name='hdn_bd' value='<?php echo $base_datos; ?>'>
				<input type='hidden' id='hdn_depto' name='hdn_depto' value='<?php echo $depto; ?>'>
				...
				<table cellpadding="5" cellspacing="5">
					...
					<td>
						...
						<td>$
							<input name="txt_subtotal" id="txt_subtotal" type="text" class="caja_de_texto"
								readonly="readonly" onkeypress="return permite(event,'num', 2);" size="15"
								maxlength="20" value="<?php echo number_format($subtotal,2,".",",");?>" />
						</td>
					</tr>
					<tr>
						<td>
							<div align="right">Requisici&oacute;n</div>
						</td>
						<td>
							<input name="txt_noReq" type="text" class="caja_de_texto" size="10" maxlength="10"
								readonly="readonly" onkeypress="return permite(event,'num_car', 3);"
						...
						<td>
							$<input name="txt_iva" id="txt_iva" type="text" class="caja_de_texto" <?php 
							if ($_POST["hdn_editable"] == "NO") {
								?> readonly="readonly" <?php
							}
							?> onkeypress="return permite(event,'num',2);" size="15" maxlength="20"
								value="<?php echo number_format($iva,2,".",",");?>" onchange="calcTotalPed(); formatCurrency(value,'txt_iva')"/>
							<input type="text" name="txt_lblIVA" id="txt_lblIVA" class="caja_de_num"
								onclick="alert ('IVA calculado en base al '+this.value);"
					...
					<tr>
						...
						<td><input name="txt_fecha" type="text" readonly="readonly" class="caja_de_texto"
								value="<?php echo verFecha(4);?>" size="18" maxlength="25" /></td>
						<td>
							<div align="right">Total </div>
						</td>
						<td>$
							<input name="txt_total" id="txt_total" type="text" class="caja_de_texto"
								onkeypress="return permite(event,'num', 2);" size="15" maxlength="20"
					...
					<tr>
						<td>
							<div align="right">*Plazo Entrega </div>
						</td>
						<td>
							<input name="txt_plazo" id="txt_plazo" type="text" class="caja_de_texto" size="4"
								maxlength="10" onkeypress="return permite(event,'num', 3);" tabindex="1"
								autocomplete="off" />
					...
					<tr>
						<td>
							<div align="right">*Condiciones Entrega </div>
						</td>
						<td>
							<textarea name="txa_condEnt" class="caja_de_texto" id="txa_condEnt" rows="2" cols="30"
								maxlength="60" onkeypress="return permite(event,'num_car', 0);"
								onkeyup="return ismaxlength(this);" tabindex="4"
								ondblclick="this.value=''">En Almac&eacute;n CLF</textarea>
						</td>
						<td>
							<div align="right">*Revis&oacute;</div>
						</td>
						<td><?php
						...
						$nombre=obtenerNombreEmpleado($rfc);?>
							...
							<input name="txt_reviso" type="text" class="caja_de_texto" id="txt_reviso" size="40"
								maxlength="60" onkeypress="return permite(event,'num_car', 2);"
								ondblclick="txt_reviso.value='';" value="<?php echo $nombre; ?>" tabindex="5" />
						</td>
					</tr>
					<tr>
						<td>
							<div align="right">*Condiciones Pago </div>
						</td>
						<td>
							<select name="cmb_condPago" class="combo_box" tabindex="6"
								onchange="agregarDescripcion(this);">
								<option value="">Seleccionar</option>
						...
						<td>
							<div align="right">*Autoriz&oacute;</div>
						</td><?php
						$rfc=obtenerDato("bd_recursos","organigrama","empleados_rfc_empleado","departamento","DIRECCION GENERAL");
						//Obtener nombre Recursos Humanos
						$nombre=obtenerNombreEmpleado($rfc);?>
						<td>
							<input name="txt_autorizo" type="text" class="caja_de_texto" size="40" maxlength="60"
								onkeypress="return permite(event,'num_car', 2);" ondblclick="txt_autorizo.value='';"
								value="<?php echo $nombre; ?>" tabindex="7" />
						</td>
					</tr>
					<tr>
						<td>
							<div align="right">*V&iacute;a del Pedido </div>
						</td>
						<td>
						...
						<td>
							<textarea name="txa_comentarios" class="caja_de_texto" id="txa_comentarios" cols="30"
								rows="2" onkeypress="return permite(event,'num_car', 0);"
					...
					<tr>
						...
						<td>
							<input name="txt_rfc" id="txt_rfc" type="text" class="caja_de_texto" readonly="readonly"
								size="20" maxlength="20" value=""
								title="Seleccionar un Proveedor Para Mostrar su RFC" />
						</td>
					...
					<tr>
						...
						$result=mysql_query("SELECT DISTINCT rfc,razon_social FROM proveedores ORDER BY razon_social");?>
							<select name="cmb_proveedor" size="1"
								onchange="document.getElementById('txt_rfc').value = this.value;" class="combo_box"
								tabindex="11">
					...
					<tr>
						<td colspan="4"><strong>* Los campos marcados con asterisco son <u>obligatorios</u></strong>
						</td>
					</tr>
					<tr>
						<td colspan="4">
							<div align="center">
								<input type="hidden" name="hdn_ivaIncluido" id="hdn_ivaIncluido"
									value="<?php echo $iva_inc;?>" />

								<input name="btn_registrarDetalle" type="submit" class="botones" value="Registrar"
									title="Registrar" onmouseover="window.status='';return true;" tabindex="12" />
								&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								<input name="rst_limpiar" type="reset" class="botones" value="Limpiar"
									title="Borrar el Formulario" tabindex="13" />
								&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								...
								<input name="btn_cancelar" type="button" class="botones" id="boton-cancelar"
									value="Cancelar" title="Cancelar"
									onclick="document.getElementById('frm_registrarPedido').action='frm_detallesDelPedido.php?depto=<?php echo $depto; ?>';document.getElementById('frm_registrarPedido').submit();"
									tabindex="15" />
								<?php }?>
								&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div>
						</td>
		...
		<div id="editar-iva">
			<input type="image" src="../../images/editar.png" width="30" height="25" border="0"
				onclick="actualizarIVA('txt_subtotal','txt_iva','txt_total');" title="Modificar la Tasa de IVA" />
		</div><?php 
