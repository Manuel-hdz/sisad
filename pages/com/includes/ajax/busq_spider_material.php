	...
	if(!$db) {
		...
		if(isset($_POST['queryString'])) {
			...
			if(strlen($queryString) >0) {
				...
				$query = $db->query("SELECT id_material, nom_material
									 FROM materiales
				...
				if($query) {
					...
					while($result = $query ->fetch_object()){// Establece los resultados a modo de lista.										
						...
						echo utf8_encode('
							<li title=\''.$result->id_material.'\' onClick="fill(\''.$_GET['nomCajaTexto'].'\' , \''.$nombreMat.'\' , \''.$_GET['num'].'\');'.$claveAlmacen.'.value='.$claveMat.'">'.$result->nom_material.'</li>');
					}// cierre while($result = $query ->fetch_object()) 
					//Indicar que no hay registros
					if($ctrlRegistros==0){
						$nombreMat = "MATERIAL NUEVO";
						...
						echo utf8_encode('
							<li onClick="fill(\''.$_GET['nomCajaTexto'].'\' , \''.$nombreMat.'\' , \''.$_GET['num'].'\');'.$claveAlmacen.'.value='.$claveMat.'">'.$msg.'</li>
						');
					}
				} else {
