	...
	if(!$db) {
		...
		if(isset($_POST['queryString'])) {
			...
			if(strlen($queryString) >0) {
				...
				$query = $db->query("SELECT CONCAT(nombre,' ',ape_pat,' ',ape_mat) AS nombre,puesto FROM bajas_modificaciones WHERE CONCAT(nombre,' ',ape_pat,' ',ape_mat) LIKE '%$queryString%' AND fecha_baja!='0000-00-00' LIMIT 10");
				if($query) {
