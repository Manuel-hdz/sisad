	...
	if(!$db) {
		...
		if(isset($_POST['queryString'])) {
			...
			if(strlen($queryString) >0) {
				...
				$query = $db->query("SELECT rfc, razon_social
									 FROM proveedores
									 WHERE razon_social LIKE  '%$queryString%'
									 OR rfc LIKE  '%$queryString%'
									 LIMIT 10");
				...
				if($query) {
					...
					while($result = $query ->fetch_object()){// Establece los resultados a modo de lista.										
						...
						$rfcProv="document.getElementById('txt_rfc_prov')";
						$rfc="'".$result->rfc."'";
						
						//Sustituir el caracter de comillas dobles en el nombre para evitar errores de cadenas no terminadas
						$razon_social=str_replace("\"","inch",$result->razon_social);
						// La función OnClick llena la caja de texto con el resultado seleccionado.
						echo utf8_encode('
							<li title=\''.$result->rfc.'\' onClick="fill(\''.$_GET['nomCajaTexto'].'\' , \''.$razon_social.'\' , \''.$_GET['num'].'\');'.$rfcProv.'.value='.$rfc.'">'.$result->razon_social.'</li>');
					...
					if($ctrlRegistros==0){
						...
						echo utf8_encode('
							<li onClick="fill(\''.$_GET['nomCajaTexto'].'\' , \''.$razon_social.'\' , \''.$_GET['num'].'\');'.$rfcProv.'.value='.$rfc.'">'.$msg.'</li>
						');
					}
				} else {
