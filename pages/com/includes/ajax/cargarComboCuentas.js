/**
  ...
  */
	...
	function cargarCuentas_Equipo(equipo, combo_con, combo_cuen, combo_sub,h_cc,h_cuen){
		cmb_con = combo_con;
		cmb_cue = combo_cuen;
		cmb_sub = combo_sub;
		hdn_cc = h_cc;
 		...
 		if(peticion_equipo_mtto.readyState==READY_STATE_COMPLETE){
			if(peticion_equipo_mtto.status==200){
				...
				if (existe=="true"){
					...
					objeto4 = document.getElementById(""+hdn_cc);
					objeto5 = document.getElementById(""+hdn_cue);
					//Recorrer la respuesta XML para colocar los valores del ComboBox
					for(var i=0;i<tam;i++){
						//Obtener cada uno de los datos que serán cargados en el Combo
					...
					objeto4.value = control_costos;
					objeto5.value = cuenta;
