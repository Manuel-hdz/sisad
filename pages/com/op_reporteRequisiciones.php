<?php
			...
			<tr>
				...
				<td align="center">
					<form action="guardar_reporte.php" method="post">
						...
						<input name="hdn_origen" type="hidden" value="reporte_requisiciones" />		
	...
	function mostrarDetalleReqArea($clave,$bd,$fecha_i,$fecha_f){
		...
		if ($clave!="todos"){
			$stm_sql = "SELECT T1.* 
						FROM  `detalle_requisicion` AS T1
						JOIN requisiciones AS T2 ON id_requisicion = requisiciones_id_requisicion
				...
				<tr>	
					<td align="center">
						<form action="frm_reporteRequisiciones.php" method="post" name="frm_reporteRequisiciones">
							...
							<input name="hdn_origen" type="hidden" value="reporte_detallerequisiciones" />		
							<input name="hdn_msg" type="hidden" value="<?php echo $msg_titulo; ?>" />
	...
	function mostrarDetalleReq($clave,$bd,$fecha_i,$fecha_f){
		$msg_titulo = "";
		$departamento = strtoupper(substr( $bd, 3));
		?><div id="reporte" class="borde_seccion2" align="center"><?php
		//Realizar la conexion a la BD de Compras
				...
				<tr>	
					<td align="center">
						<form action="frm_reporteRequisiciones.php" method="post" name="frm_reporteRequisiciones">
							...
							<input name="hdn_origen" type="hidden" value="reporte_detallerequisiciones" />		
							<input name="hdn_msg" type="hidden" value="<?php echo $msg_titulo; ?>" />
							<input type="hidden" name="cmb_departamento" id="cmb_departamento" value="TODOS" />
							<input type="hidden" name="hdn_clave" id="hdn_clave" value="<?php echo $clave; ?>" />
							<input type="hidden" name="txt_fecha_ini" id="txt_fecha_ini" value="<?php echo modFecha($fecha_i,1); ?>" />
							<input type="hidden" name="txt_fecha_fin" id="txt_fecha_fin" value="<?php echo modFecha($fecha_f,1); ?>" />
							<input name="btn_exportar" type="button" class="botones" value="Exportar a Excel" title="Exportar el Reporte a Excel" 
		...
		if ($clave=="todos"){
			...
			while($db = mysql_fetch_row($rs_db)){
				if(substr( $db[0], 0, 3 ) == "bd_"){
					...
					$msg_titulo = "DETALLE DE LA REQUISICIONES DEL DEPARTAMENTO: <em><u>TODOS</u></em> DEL <em><u>".modFecha($fecha_i,1)."</u></em> AL <em><u>".modFecha($fecha_f,1)."</u></em>";
					$rs = mysql_query($stm_sql);
					if($rs){
						if($datos=mysql_fetch_array($rs)){
							if($aux == 0){
	...
	function mostrarRequisiciones($bd,$fecha_i,$fecha_f,$msg_titulo){
		$flag = 0;
