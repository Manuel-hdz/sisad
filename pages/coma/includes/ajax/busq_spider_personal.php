	...
	if(!$db) {
		...
		if(isset($_POST['queryString'])) {
			...
			if(strlen($queryString) >0) {
				...
				$query = $db->query("SELECT CONCAT(nombre,' ',ape_pat,' ',ape_mat) AS nombre,puesto,id_empleados_empresa FROM empleados WHERE CONCAT(nombre,' ',ape_pat,' ',ape_mat) LIKE '%$queryString%' AND estado_actual = 'ALTA' LIMIT 10");
				if($query) {
					...
					while($result = $query ->fetch_object()){// Establece los resultados a modo de lista.
						...
						echo utf8_encode('
								<li title='.$puesto.' onClick="fill(\''.$_GET['nomCajaTexto'].'\' , \''.$result->nombre.'\' , \''.$_GET['num'].'\'); fill(\''.'txt_codBarTrabajador'.'\' , \''.$result->id_empleados_empresa.'\' , \''.$_GET['num'].'\');">'.$result->nombre.'</li>
							');
