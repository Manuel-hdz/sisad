...
<head>
    ...
    <style type="text/css">
		<!--
			#titulo-consultar{position:absolute;left:30px;top:146px; width:375px;height:20px;z-index:11;}
...
<body>
	...
	<?php 
		...
		if (isset($_POST["cmb_equipo"])){
			if ($_POST["cmb_equipo"]=="JUMBO")
				$tipo="con Jumbo";
			else if ($_POST["cmb_equipo"]=="MP")
				$tipo="con M&aacute;quina de Pierna";
			else
				$tipo="Anclas";
				
		}
	?>
    <div class="titulo_barra" id="titulo-consultar">Reporte de Barrenaci&oacute;n <?php echo $tipo?></div>
	<?php 
	...
	if (!isset($_POST["sbt_consultar"])){
		...
		<br>
		<form name="frm_consultarReporte" method="post" action="frm_reporteBarrenacion.php"  onsubmit="return valFormConsultarReporteCombo(this);">
			<table width="444"  cellpadding="5" cellspacing="5" class="tabla_frm">
				...
				<tr>
					<td width="104"><div align="right">Tipo Equipo</div></td>
					<td>
						<select name="cmb_equipo" id="cmb_equipo" class="combo_box">
							<option value="" selected="selected">TIPO EQUIPO</option>
							<option value="JUMBO">JUMBOS</option>
							<option value="MP">M&Aacute;QUINAS DE PIERNA</option>
							<!-- <option value="ANCLA">ANCLAS</option> -->
						</select>
					</td>
				</tr>
				<tr>
	...
	?>
		<form name="frm_consultarResultados" method="post" action="guardar_reporte.php"><?php 
			echo"<div id='tabla-resultados' class='borde_seccion2' align='center'>";
				if ($_POST["cmb_equipo"]=="JUMBO"){
					$tipoRpt="reporteBarrenacionJum";
					$res=mostrarBarrenacionJum();
				}
				else if ($_POST["cmb_equipo"]=="MP"){
					$tipoRpt="reporteBarrenacionMP";
					$res=mostrarBarrenacionMP();
				}
				else {
					$tipoRpt="reporteBarrenacionAnc";
					$res=mostrarBarrenacionJum();
				}
			echo "</div>";
			...
			if ($res!="false"){
				...
				?>
					...
					<input type="hidden" name="fecha_ini" id="fecha_ini" value="<?php echo $datos[3];?>"/>
					<input type="hidden" name="fecha_fin" id="fecha_fin" value="<?php echo $datos[4];?>"/>
					<input type="hidden" name="hdn_msje" id="hdn_msje" value="<?php echo $datos[1];?>"/>
					<input type="hidden" name="hdn_tipoReporte" id="hdn_tipoReporte" value="<?php echo $tipoRpt;?>"/>
				...
				onMouseOver="window.status='';return true" onclick="location.href='frm_reporteBarrenacion.php'" />
			</div>
		</form>
	<?php }?>
</body>
