...
<head>
    ...
    <style type="text/css">
		<!--
			...
			#calendario-uno{position:absolute;left:325px;top:230px;width:30px;height:26px;z-index:13;}
			#calendario-dos{position:absolute;left:325px;top:266px;width:30px;height:26px;z-index:14;}
...
<body>
		...
		<form name="frm_reporteNomina" method="post" action="frm_reporteNomina.php" onsubmit="return valGenerarNomina(this);">
			<table width="415"  cellpadding="5" cellspacing="5" class="tabla_frm">
				<tr>
				  ...
				  <?php /* 
				  <td width="60"><div align="right">&Aacute;rea:</div></td>
					<td width="83">
					<?php
					//Evitar que la variable $cmb_categoria marque un error por no estar definida			
					if(!isset($_POST['cmb_area'])) $cmb_area = "";
					else $cmb_area = $_POST["cmb_area"];
					$conn = conecta("bd_desarrollo");
					$rs = mysql_query("SELECT DISTINCT area FROM catalogo_salarios ORDER BY area");
					if($row=mysql_fetch_array($rs)){?>            
						...
						<label class="msje_correcto"><u><strong>NO</strong></u> Hay &Aacute;reas Para Generar N&oacute;mina</label>
				...
				<tr>
					...
					<td>
						<input name="txt_fechaFin" type="text" id="txt_fechaFin" size="10" maxlength="15"
	...
	?>
		...
		<div id="botones" align="center">
			<form name="frm_consultarResultados" action="guardar_reporte.php" method="post">
				<?php 
				if($sql!="false"){
