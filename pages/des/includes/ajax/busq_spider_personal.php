	...
	if(!$db) {
		...
		if(isset($_POST['queryString'])) {
			...
			if(strlen($queryString) >0) {
				...
				$query = $db->query("SELECT rfc_empleado,puesto,CONCAT(nombre,' ',ape_pat,' ',ape_mat) AS nombre FROM $_GET[nomTabla] WHERE CONCAT(nombre,' ',ape_pat,' ',ape_mat) LIKE '%$queryString%' AND area='DESARROLLO FRESNILLO' AND id_cuentas LIKE 'CUEN001' AND estado_actual = 'ALTA' ORDER BY nombre LIMIT 10");
				if($query) {
