<?php
	...
	**/
	...
	function reporteKardexChecador(){
		...
		$fechaIini=modFecha($_POST["txt_fechaIni"],3);
		...
		if(isset($_POST["cmb_area"])){
			...
			if ($area=="TODOS"){
				//Sentencia SQL para extraer a los Trabajadores del Área Seleccionada
				$sql = "SELECT T1. * , CONCAT( T2.ape_pat, ' ', T2.ape_mat, ' ', T2.nombre ) AS nombre, T2.id_control_costos
						FROM `detalle_checador` AS T1
						JOIN empleados AS T2 ON T1.id_empleado = T2.id_empleados_empresa
						WHERE fecha
						BETWEEN '$fechaIini'
						AND '$fechaFin'
						ORDER BY nombre, T1.fecha, T1.hora";
			}
			else{
				$sql = "SELECT T1. * , CONCAT( T2.ape_pat, ' ', T2.ape_mat, ' ', T2.nombre ) AS nombre, T2.id_control_costos
						FROM `detalle_checador` AS T1
				...
				if( isset($_POST['txt_cuenta']) ){
					if( $_POST['txt_cuenta'] == "PRODUCCION" ){
						$sql .= " AND T2.id_cuentas NOT LIKE 'CUEN002'";
					}
				}

				$sql .=	" ORDER BY nombre, T1.fecha, T1.hora";
			}
		}
		else{//Consulta usando Filtro por Nombre de Trabajador
			$nombre=$_POST["txt_nombreK"];
		...
		if($rs = mysql_query ($sql)){
			...
			for($i=0; $i<$num_reg; $i+=2){
				...
				if($id_emp != $empleados[$i]["id_empleado"]){
					...
					echo "
						<thead>
							...
							<tr>
								<th class='nombres_columnas' align='center'>".$empleados[$i]['id_empleado']."</th>
								<th class='nombres_columnas' align='center' colspan='4'>".$empleados[$i]['nombre']."</th>
							</tr>
							<tr>
								<th class='nombres_columnas' align='center'>D&Iacute;A</th>
								<th class='nombres_columnas' align='center'>FECHA</th>
								<th class='nombres_columnas' align='center'>ENTRADA / SALIDA</th>
				...
				echo "
					<tr>
					<td class='$nom_clase' align='center'>".obtenerNombreDia($empleados[$i]['fecha'])."</td>
					<td class='$nom_clase' align='center'>".modFecha($empleados[$i]['fecha'],1)."</td>
					<td class='$nom_clase' align='center'>".$empleados[$i]['hora']."</td>
					<td class='$nom_clase' align='center' rowspan='2'>".number_format($horas_trab,2,".",",")."</td>
					<td class='$nom_clase' align='center' rowspan='2'>".number_format($extras,2,".",",")."</td>
					</tr>";
				...
				if($horas_dia < 14 && $empleados[$i+1]["id_empleado"] == $empleados[$i]["id_empleado"]){
					...
					echo "
						<tr>
						<td class='$nom_clase' align='center'>".obtenerNombreDia($empleados[$i+1]['fecha'])."</td>
						<td class='$nom_clase' align='center'>".modFecha($empleados[$i+1]['fecha'],1)."</td>
						<td class='$nom_clase' align='center'>".$hora_fin."</td>
						</tr>";	
				} else {
					$hora_fin = "00:00:00";
					echo "
