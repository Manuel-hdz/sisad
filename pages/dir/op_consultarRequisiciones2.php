<?php
	...
	//Esta funcion se encarga de guardar el comentario escrito para las requisiciones, además del estado asignado
	 function guardarComentario($idReq,$comentario,$departamento,$base){
		...
		$conn = conecta($base);
		//Crear sentencia SQL
		$stm_sql="UPDATE requisiciones SET comentario_aut='$comentario' WHERE id_requisicion='$idReq'";
		//Ejecutar la sentencia previamente creada
			$rs = mysql_query($stm_sql);
		if ($rs){
			/*?>
		...
		if (!$conec){
			...
			echo "
					...
					<tr align='center'>
						...
						<td class='nombres_columnas'>AUTORIZAR</td>
				...
				if(isset($_POST["hdn_numero"])){
					...
					<script type="text/javascript" language="javascript">
						...
						function focoReq(combo){
							...
							document.getElementById(renglon).style.background="#CC66CC";
