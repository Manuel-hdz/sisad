...
<div class="titulo_barra" id="titulo-eliminar">Eliminar Material </div>

<?php //Si no estan definidas las variables $hdn_clave, $cmb_datoBuscar y $rdb_clave mostrar los formularios para seleccionar el material a eleminar y el formulario para buscar
	$band = 0;
		...
		<tr>
			...
			<td width="241"><?php if($aux==1){?>
			  <select name="cmb_material" size="1" onchange="javascript:document.frm_cargarInfoCombos.submit();" class="combo_box">
                ...
                <?php $result1 = mysql_query("SELECT id_material,nom_material FROM materiales WHERE linea_articulo='$cmb_categoria' AND grupo='PLANTA'");		
					$band = 0;
	...
	<table border="0" cellpadding="5" cellspacing="5" class="tabla_frm">
		...
		<tr>
			...
			<td><input name="btn_cancelar" type="button" class="botones" value="Cancelar" title="Regresar al Men&uacute; de Materiales" onClick="location.href='menu_materiales.php'" /></td>
		</tr>
	</table>      
	</form>		
</fieldset>
	...
	<legend class="titulo_etiqueta">Eliminar Material Por Clave </legend>
	<?php
		...
		if(!isset($_POST["sbt_enviarClave"]))
			...
			if ($grupo!="PLANTA")
				$msj="Material Administrado por Almacén No se Puede Eliminar";	
