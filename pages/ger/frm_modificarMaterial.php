...
<head>
    ...
    <style type="text/css">
		...
		#form-modificar { position:absolute; left:30px; top:190px; width:940px; height:509px; z-index:12; }
		-->
    </style>
</head>
<body>
...
<?php //Si las variables $cmb_material y $txt_clave no estan definidas mostrar los formularios para seleccionar el material a modificar
	...
	<table border="0" cellpadding="5" cellspacing="5" class="tabla_frm">		
		...
		<tr>
			...
			<select name="cmb_material" size="1" class="combo_box" id="cmb_material">
				...
				<?php 
				$result1 = mysql_query("SELECT id_material,nom_material FROM materiales WHERE linea_articulo='$cmb_categoria'  AND grupo='PLANTA' ORDER BY nom_material");		
		...
		<tr>
			<td colspan="2">
		  		<div align="center">
					...
					&nbsp;&nbsp;&nbsp;
					<input name="btn_cancelar" type="button" class="botones" value="Cancelar" title="Regresar al Men&uacute; de Materiales" onClick="location.href='menu_materiales.php'" />
				</div>
			</td>
		</tr>
		</form>	      
	...
	//Si no se ha presionado el boton, no se muestra ningun mensaje
		if(!isset($_POST["sbt_enviarClave"]))
			...
			//Si el grupo es diferente de planta, el material no puede ser Modificado por el Adminitrador de Gerencia
			if ($grupo!="PLANTA")
				$msj="Material Administrado por Almacén, No se Puede Modificar";	
	...
	<form method="post" name="frm_modificarXClave" action="" onsubmit="return valFormModificarXClave(this);">
		...
		<tr>
			...
			<td width="289">
 				<input type="text" name="txt_claveMod" id="txt_claveMod" size="10" maxlength="10"/>
		  </td>
		</tr>
	...
	<?php 
		...
		if(isset($_POST['txt_clave'])){
			...
			if(isset($_POST['cmb_lineaArticulo']))
				...
				$lineaArticulo = $txt_lineaArticulo;
			...
			if(isset($_POST['cmb_unidadMedida']))
				...
				$unidadMedida = $txt_unidadMedida;
				
			guardarCambios($txt_clave,$txt_nombre,$txt_nivelMinimo,$txt_nivelMaximo,$txt_puntoReorden,$txt_costoUnidad,$lineaArticulo,$unidadMedida,$cmb_proveedor,$txt_ubicacion,$txa_comentarios,$txt_factor,$txt_unidadDespacho,$foto,$cmb_relevancia);
		}
	}//Cierre del Else ?>
</body>
