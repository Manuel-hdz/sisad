/**
  ...
  */
	...
	/******************************************************************************************
	 * Esta función cargara en un combo box los  periodos asoicadas a un destino seleccionado
	 ******************************************************************************************/
	function cargarComboPeriodo(destino){		
		...
		if(destino!=""){			
			...
			//incluido este archivo JavaScript(cargarComboGT.js)
			var url = "includes/ajax/cargarComboGT.php?destino="+destino;		
			...
			cargaContenidoCombo(url, "GET", procesarRespuestaCombo);
		}
	...
	function procesarRespuestaCombo(){				
		//Verificar que la peticion HTTP se haya realizado correctamente
 		if(pet_http_cmb.readyState==READY_STATE_COMPLETE){
			if(pet_http_cmb.status==200){
				//Recuperar la respuesta del Servidor
				var respuesta = pet_http_cmb.responseXML;
	...
	function cargaContenidoCombo(url, metodo, funcion) {
		pet_http_cmb = inicializa_xhr_combo();
		if(pet_http_cmb){
			pet_http_cmb.onreadystatechange = funcion;
			pet_http_cmb.open(metodo, url, true);
			pet_http_cmb.send(null);
		}
	}
	
	
	/*Esta funcion encapsula la creación del objeto XMLHttpRequest*/
	function inicializa_xhr_combo() {		
