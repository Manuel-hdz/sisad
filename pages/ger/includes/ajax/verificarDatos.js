/**
  ...
  */
	...
	function verificaRegistros(nomTrabajador,fechaRegistro){
		//Verificar que sea proporcionado el Nombre de un Trabajador
		if(nomTrabajador!=""){
			...
			var url = "includes/ajax/verificarDatos.php?nombre="+nomTrabajador+"&fecha="+fechaRegistro;
		...
		//Verificar que la peticion HTTP se haya realizado correctamente
 		if(pet_http_vd.readyState==READY_STATE_COMPLETE){
			if(pet_http_vd.status==200){
				...
				if (existe=="true"){		
					//Recuperar el Nombre del Trabajador y la Fecha
					...
					var cantRegistros = respuesta.getElementsByTagName("registros").item(0).firstChild.data;
					
					//EN caso de que el Trabajador tenga registros, solo notificar al Usuario
					alert("El Trabajador "+nombre+" \ntiene "+cantRegistros+" Registro(s) de Transporte en la Fecha "+fecha);
				}
				else{	
					//en caso de que no tenga registros, no hacer nada
				}
