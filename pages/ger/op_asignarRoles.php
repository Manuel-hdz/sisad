<?php
	/**
	  * Nombre del Módulo: Recursos Humanos-Gerencia Técnica
	  * Nombre Programador: Antonio de Jesús Jiménez Cuevas
	  * Fecha: 14/Marzoo/2012
	...
	**/
	...
	function mostrarTrabajadores(){
		...
		$stm_sql="SELECT rfc_empleado,CONCAT(nombre,' ',ape_pat,' ',ape_mat) AS nombre,area,puesto FROM empleados WHERE area='CONCRETO' AND id_empleados_empresa>0 ORDER BY puesto,id_empleados_empresa";
		...
		do{
			if (isset($_POST["ckb_seleccionar".$cont])){
				...
				if (!$rs){
					...
					registrarOperacion("bd_gerencia","$rfc","ActualizarRol",$_SESSION['usr_reg']);
				}
			}
			$cont++;
		}while($cont<=$tam);
