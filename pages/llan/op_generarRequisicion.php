<?php
	/**
	  * Nombre del Módulo: Llantas                                              
	  * Nombre Programador: Antonio de Jesús Jiménez Cuevas                            
	  * Fecha: 16/Noviembre/2012                                     			
	  * Descripción: Este archivo contiene funciones para almacenar la información relacionada con el formulario de Generar Requisicones 
	  **/
	...
	function obtenerIdRequisicion(){
		$id_cadena="tmp15695879";
		return $id_cadena;
	}//Fin de la Funcion obtenerIdRequisicion()
	
	function obtenerIDReqReal($id_cadena){
		...
		$stm_sql = "SELECT COUNT(id_requisicion) AS cant FROM requisiciones WHERE id_requisicion LIKE '$id_cadena%'";
	...
	function guardarRequisicion($txa_justificacionReq,$hdn_fecha,$txt_areaSolicitante,$txt_solicitanteReq,$txt_elaboradorReq){
		if($txt_areaSolicitante=="MANTENIMIENTO CONCRETO")
			$_SESSION["id_requisicion"]=obtenerIDReqReal("MAC");
		else
			$_SESSION["id_requisicion"]=obtenerIDReqReal("MAM");
		//Realizar la conexion a la BD de Paileria
		$conn = conecta("bd_mantenimiento");
		...
		$txt_solicitanteReq = strtoupper($txt_solicitanteReq);
		...
		if($rs){			
			...
			foreach ($_SESSION['datosRequisicion'] as $ind => $material) {			
				...
				$stm_sql = "INSERT INTO detalle_requisicion (requisiciones_id_requisicion, cant_req, unidad_medida, descripcion, aplicacion,
				materiales_id_material)
				VALUES('$_SESSION[id_requisicion]', $material[cantReq], '$material[unidad]', '$material[material]', '$material[aplicacionReq]', '$material[clave]')";
			...
			if($band==0){
				...
				registrarOperacion("bd_mantenimiento",$_SESSION['id_requisicion'],"GenerarRequisicion",$_SESSION['usr_reg']);
	...
	function mostrarRegistros($datosRequisicion){
		...
		<table cellpadding='5' align='center'>      			
			<tr>
				...
				<td width='120' class='nombres_columnas' align='center'>FOTOGRAF&Iacute;A</td>				
				<td width='30' class='nombres_columnas' align='center'>EDITAR</td>
      		</tr>";
					...
					case "aplicacionReq":
						...
						if($claveMat!="N/A"){?>
							<td class="<?php echo $nom_clase; ?>">
								<span >
		    						<input type="button" name="btn_foto" class="botones" value="Agregar Foto" 
									onmouseover="window.estatus='';return true" title="Agregar Foto a la Requisici&oacute;n" 
									onclick="javascript:window.open('verRegistrarFotoRequisicion.php?nombreMat=<?php echo $claveMat;?>',
					'_blank','top=50, left=50, width=620, height=250, status=no, menubar=no, resizable=no, scrollbars=yes, toolbar=no, location=no, directories=no');"/>
		  						</span>
							</td><?php }
						else{?>
						<td class="<?php echo $nom_clase; ?>">
								<span >
		    						<input type="button" name="btn_foto" class="botones" value="Agregar Foto" 
									onmouseover="window.estatus='';return true" title="No puede Agregar Foto a un Material no Registrado"  disabled="disabled"/>
		  						</span>
							</td>
						<?php }
					break;
	...
	//Esta función verifica que no se duplique un registro en el arreglo que guarda los datos del Detalle de la Requisición
	...
	function subirFotos($clave, $nombre){
		...
		if($nomArchivo!= 'JPG'){
			?>
				<script>
					setTimeout("alert('El Formato del Archivo no es Válido. Sólo se Permiten Imágenes JPG');",500);
				</script>
			<?php
			exit('');
	...
	function borrarFotosSesionLlantas(){
		//Almacenamos el valor de la clave de la requisicion
		$carpeta=$_SESSION["fotosReq"][0]["clave"];
		//Borrar los ficheros temporales
		$h=opendir('llan/documentos/'.$carpeta);
		while ($file=readdir($h)){
			if (substr($file,-4)=='.jpg'){
				@unlink("llan/documentos/".$carpeta."/".$file);
			}
		}
	}//Fin de la funcion borrarFotosSesionDesarrollo()
?>