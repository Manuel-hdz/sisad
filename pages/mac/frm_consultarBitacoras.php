		...
		<table border="0" align="center" cellpadding="5" cellspacing="5" class="tabla_frm">
			<tr>
				...
				<td width="215"><?php 
					...
					$dpto = "";//Guardar el nombre del Area de acuerdo al Usuario registrado en la SESSSION
					$sql_stm_ot = ""; //Guardar la consulta segun el usuario registrado
					
					//Verificamos que usuario viene definido en la session para crear la Sentencia SQL correspondiente
					...
					if($_SESSION["depto"]=="MttoMina" || $_SESSION["depto"]=="MttoConcreto"){
						$sql_stm_ot = "SELECT id_orden_trabajo FROM ((orden_trabajo JOIN bitacora_mtto ON orden_trabajo_id_orden_trabajo=id_orden_trabajo)
										JOIN equipos ON bitacora_mtto.equipos_id_equipo=equipos.id_equipo) WHERE equipos.area='$dpto' ORDER BY id_orden_trabajo";
					}
					else{//Crear la Sentencia cuando el usuario registrado tenga acceso a Ambas areas
						$sql_stm_ot = mysql_query("SELECT id_orden_trabajo FROM orden_trabajo ORDER BY id_orden_trabajo");
					}
					...
					$rs_ot = mysql_query($sql_stm_ot);
					//Verificar si Sentencia ejecutada regresa datos	
					if($datos_ot=mysql_fetch_array($rs_ot)){?>
						<select name="cmb_OT" id="cmb_OT" size="1"  class="combo_box" onchange="javascript:document.frm_consultarBitacora.submit();">
							...
							do{
								if($datos_ot['id_orden_trabajo'] == $cmb_OT){
									echo "<option value='$datos_ot[id_orden_trabajo]' selected='selected'>$datos_ot[id_orden_trabajo]</option>";
								}
								else{
									echo "<option value='$datos_ot[id_orden_trabajo]'>$datos_ot[id_orden_trabajo]</option>";
						   		}	
							}while($datos_ot=mysql_fetch_array($rs_ot));?>
						</select> <?php
					}
					else{
			...
			if(isset($_POST['cmb_OT'])){?>
				<tr>
					...
					<td width="173"><?php 
						//Obtenemos datos segun la orden de trabajo seleccionada						
						$id_bitacora = obtenerDato("bd_mantenimiento", "bitacora_mtto", "id_bitacora", "orden_trabajo_id_orden_trabajo", $_POST["cmb_OT"]);?>
						<input name="txt_bitacora" id="txt_bitacora" type="text" class="caja_de_texto" size="15" value="<?php echo $id_bitacora;?>" readonly="readonly" /> 
					</td>
				</tr>
				<tr>
					<td><div align="right">Clave Equipo</div></td>
					<td width="173"><?php 
						//Obtener la Clave del Equipo registrada en la Orden de Trabajo				
						$txt_claveEquipo = obtenerDato("bd_mantenimiento", "bitacora_mtto", "equipos_id_equipo", "orden_trabajo_id_orden_trabajo",$_POST["cmb_OT"]);?>						
						<input name="txt_equipo" id="txt_equipo" type="text" class="caja_de_texto" size="15" readonly="readonly" value="<?php echo $txt_claveEquipo?>" /> 
					</td>
				</tr><?php 
			}?>
			<tr>                    
				<td align="center" colspan="2"><?php 
					if(isset($_POST['cmb_OT'])){?>
						<input name="sbt_consultar" type="submit" class="botones" value="Consultar" onMouseOver="window.estatus='';return true" title="Consultar Bit&aacute;cora" />
						&nbsp;&nbsp;<?php 
					}?>
