...
<body>
	...
	if(!isset($_POST['sbt_consultar'])){
		...
		<table width="451" cellpadding="5" cellspacing="5" class="tabla_frm">
			<tr>
				...
				<td width="78"><input name="cmb_area" id="cmb_area" type="text" class="caja_de_texto" size="10" value="<?php echo $area;?>" 
					readonly="readonly"/>
                </td>
			</tr>
            <tr>
				...
				<td>
					<?php 
						$val = cargarComboBicondicional("cmb_familia","familia","equipos","bd_mantenimiento",$area,"area","ACTIVO","estado","Familia","","cargarEquiposFamilia(this.value,'$area','cmb_claveEquipo','Clave','')");
						if($val==0){?>
			...
			<td><div align="right">Equipo</div></td>            
				<td>
					...
					<?php } 
						...
						<input type="hidden" name="cmb_familia" id="cmb_familia" value=""/>
						<label class='msje_correcto'> No hay Equipos Registrados</label>
					<?php }?>
                </td> 
	...
	if(isset($_POST['sbt_consultar'])){?>
		<form onSubmit="return valFormModHoro(this);" method="post" action="frm_consultarHoroOdometro.php" name="frm_modHoroMetrica">
    	<div id="verMetrica" align="center" class="borde_seccion2"><?php
			mostrarMetrica()?>
        </div>
        <div id="btn_reg" align="center">
			<input name="sbt_modificar" type="submit" class="botones"  value="Modificar" title="Modificar Horometros" onmouseover="window.status='';return true"/>
            &nbsp;&nbsp;&nbsp;&nbsp;
