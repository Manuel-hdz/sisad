...
<head>
    ...
    <style type="text/css">
		...
		#calendario {position:absolute;left:566px;top:355px;width:30px;height:26px;z-index:13;}
...
<body><?php
	...
	//Sí se le dio clic al boton cancelar en la pagina donde se asocian las gamas a la OT, quitar de la SESSION el arreglo de 'gamasOT'
	...
	if(!isset($_POST['sbt_generarOrdenTrabajo'])){
		...
		if(!isset($_POST['sbt_continuar'])){ ?>		 
			...
			if(!isset($_SESSION['datosOT'])){	
				if($estado==0 && $ctrl_alerta == 0){?>
					<script type="text/javascript" language="javascript">
						cargarCombo('<?php echo $area;?>','bd_mantenimiento','gama','familia_aplicacion','area_aplicacion','cmb_familia','Familia');
					</script><?php 
				} 
			}//Cierre if(!isset($_SESSION['datosOT']))?>
                    ...
                    <td width="144"><?php
						if (!isset($_SESSION['gamasOT'])) {?>
							<select name="cmb_servicio" id="cmb_servicio" class="combo_box" onchange="varificarTipoServicio();">
								...
								value="<?php echo $cmb_servicio;  ?>"/>	<?php 
						}?>
					</td>
					<!--<td width="131"><div align="right">Id Orden de Trabajo</div></td>-->
					<!--<td width="157">-->
						<input name="txt_ordenTrabajo" type="hidden" class="caja_de_texto" readonly="readonly" id="txt_ordenTrabajo"
						onkeypress="return permite(event,'num_car',3);" size="10" maxlength="10" 
				...
				<tr>
					...
					<td><?php 
						if (!isset($_SESSION['gamasOT'])) {
                            ...
                            value="<?php echo $cmb_area;  ?>"/>	<?php 
						} ?>                    
					</td>            
				  	<td><div align="right">Fecha Registro</div></td>
					<td><input type="text" name="txt_fechaOrdenTrabajo" id="txt_fechaOrdenTrabajo" size="10" maxlength="10" class="caja_de_texto" readonly="readonly" 
					   value="<?php echo $txt_fechaOrdenTrabajo; ?>" />
					</td>
			 		...
			 		<td width="174"><?php
						if (!isset($_SESSION['gamasOT'])) {?>
							...
							onchange="formatCurrency(this.value,'txt_cantidadMetrica');" /><?php 
								...
								value="<?php echo number_format($txt_cantidadMetrica,2,".",",");  ?>"/><?php 
						}?>                    
					</td>
                </tr>
				<tr>
					...
					<td><?php
						if (!isset($_SESSION['gamasOT'])) {?>
                            <select name="cmb_familia" id="cmb_familia" onchange="cargarEquiposFamilia(this.value,'<?php echo $area; ?>','cmb_claveEquipo','Clave','');">
								...
								value="<?php echo $cmb_familia;  ?>"/><?php 
						} ?>                    
					</td>
					<td><div align="right">Fecha Programada</div></td>
					<td>
						<input type="text" name="txt_fechaProgramada" id="txt_fechaProgramada" size="10" maxlength="10" class="caja_de_texto" readonly="readonly" 
						value="<?php echo $txt_fechaProgramada; ?>">					
					</td>
				...
				<tr>
					...
					<td>
						...
						if (!isset($_SESSION['gamasOT'])) {?>
							...
							value="<?php echo $cmb_claveEquipo;  ?>"/><?php 
						}?>
					</td>
					...
					<td><?php
						if (!isset($_SESSION['gamasOT'])) {?>
                            ...
                            onkeypress="return permite(event,'num_car',0);" value="<?php echo $txt_operadorEquipo; ?>" /><?php 
						}
						else{?>
							<input name="txt_operadorEquipo" type="text" class="caja_de_texto"  size="18" readonly="readonly" id="txt_operadorEquipo"
							value="<?php echo $txt_operadorEquipo;  ?>"/><?php 
						} ?>                    
					</td>
					...
					<td><?php
						if (!isset($_SESSION['gamasOT'])) {?>	
							...
							<textarea name="txa_comentarios" id="txa_comentarios"   maxlength="120" 
							class="caja_de_texto" rows="2" readonly="readonly" cols="30"><?php echo $txa_comentarios; ?></textarea><?php 
						}?>
					</td>
				</tr>
				<tr>
					...
					<td colspan="3"><?php
						if (!isset($_SESSION['gamasOT'])) {?><?php						
                            ...
                            $result = mysql_query("SELECT departamento,CONCAT(nombre,' ',ape_pat,' ',ape_mat) AS administrador 
													FROM empleados JOIN organigrama ON empleados_rfc_empleado=rfc_empleado ORDER BY departamento");?>
                            ...
                            <select name="cmb_autorizoOT" size="1" class="combo_box">
								...
								value="<?php echo $cmb_autorizoOT;  ?>"/><?php 
						} ?>	                
					</td>						
					...
					<td><?php 
						if(!isset($_SESSION['gamasOT'])){?>
                        	...
                        	onkeypress="return permite(event,'num_car',0);" value="<?php echo $txt_proveedor; ?>" readonly="readonly" /><?php 
						} 
						else{ ?>
							<input name="txt_proveedor" id="txt_proveedor" type="text" class="caja_de_texto" size="30" maxlength="60" 
                        	readonly="readonly" value="<?php echo $txt_proveedor; ?>" /><?php 
						}?>
					</td>
				</tr>
				<tr><?php 
					if($ast=="0"){?>
						<td colspan="2"><strong>* Los campos marcados con asterisco son <u>obligatorios</u></strong></td><?php 
					}?>
				</tr>
				<tr>
				  	<td colspan="6"><div align="center"><?php 
						if (isset($_SESSION['gamasOT'])){?>
							...
							&nbsp;&nbsp;&nbsp;
							<input name="btn_impGama" id="btn_impGama" type="button" class="botones_largos"  value="Imprimir Actividades" 
							title="Imprimir las Actividades de la Gama Seleccionada" onClick="location.href='imprimirGamas.php?btn_impGama'"/>
							&nbsp;&nbsp;&nbsp;<?php 
						} 
						if (!isset($_SESSION["datosOT"]) || !isset($_SESSION['gamasOT'])){?>
						...
						if (!isset($_SESSION['gamasOT'])) {?> 
							...
							onmouseover="window.status='';return true" onclick="location.href='menu_ordenTrabajo.php'" />
							&nbsp;&nbsp;&nbsp;<?php 
						} 
						else { ?>
							<input name="btn_cancelar" type="button" class="botones" value="Cancelar" title="Cancelar Orden de Trabajo " 
							onmouseover="window.status='';return true" onclick="confirmarSalida('menu_ordenTrabajo.php');" /><?php 
						}?>				
						</div>  
					</td>
				</tr>
		...
		else{//Cuando se le da click al boton continuar cargar los datos de la OT en la SESSION y Redireccionar a la página donde se agregan las gamas
			...
			$_SESSION['datosOT'] = array ("servicio"=>$_POST['cmb_servicio'], "area"=>$_POST['cmb_area'], "familia"=>$_POST['cmb_familia'],
			 ...
			 "comentarios"=> strtoupper($_POST['txa_comentarios']), "autorizoOT"=>$_POST['cmb_autorizoOT'], "proveedor"=>strtoupper($_POST["txt_proveedor"]));
