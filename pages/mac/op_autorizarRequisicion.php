<?php
	function mostrarRequisiciones(){
		...
		<table cellpadding='5' cellspacing='5' class='tabla_frm' width='100%'>
			<tr>
				<td colspan='4' align='center' class='titulo_etiqueta'>Requisiciones Publicadas por MANTENIMIENTO ZARPEO entre el <?php echo modFecha($fechaI,2)." y el ".modFecha($fechaF,2); ?></td>
			</tr>
			<tr>
				<td colspan='4'>&nbsp;</td>
	      	</tr>
			...
			if($filtro==""){
				$stm_sql = "SELECT id_requisicion, fecha_req, hora, estado, prioridad, cant_entrega, tipo_entrega FROM requisiciones JOIN bitacora_movimientos ON id_requisicion = id_operacion 
							WHERE fecha_req>='$fechaI' AND fecha_req<='$fechaF' AND id_requisicion LIKE 'MAC%' AND bitacora_movimientos.tipo_operacion =  'GenerarRequisicion' AND autorizada = 0 ORDER BY id_requisicion";
			} else {
				if($tipoFiltro=="descripcion"){
					$stm_sql = "SELECT DISTINCT(id_requisicion), fecha_req, hora, requisiciones.estado, prioridad, requisiciones.cant_entrega, requisiciones.tipo_entrega FROM requisiciones 
								...
								WHERE fecha_req>='$fechaI' AND fecha_req<='$fechaF' AND id_requisicion LIKE 'MAC%' AND descripcion LIKE '%$filtro%' AND bitacora_movimientos.tipo_operacion =  'GenerarRequisicion' 
				...
				if($tipoFiltro=="aplicacion"){
					$stm_sql = "SELECT DISTINCT(id_requisicion), fecha_req, hora, requisiciones.estado, prioridad, requisiciones.cant_entrega, requisiciones.tipo_entrega FROM requisiciones 
								JOIN detalle_requisicion ON id_requisicion=requisiciones_id_requisicion JOIN bitacora_movimientos ON id_requisicion = id_operacion 
								WHERE fecha_req>='$fechaI' AND fecha_req<='$fechaF' AND id_requisicion LIKE 'MAC%' AND aplicacion LIKE '%$filtro%' AND bitacora_movimientos.tipo_operacion =  'GenerarRequisicion' 
								AND autorizada = 0 ORDER BY id_requisicion";
				}
				...
				if($tipoFiltro=="justificacion_tec"){
					$stm_sql = "SELECT DISTINCT(id_requisicion), fecha_req, hora, estado, prioridad, requisiciones.cant_entrega, requisiciones.tipo_entrega FROM requisiciones 
								...
								WHERE fecha_req>='$fechaI' AND fecha_req<='$fechaF' AND id_requisicion LIKE 'MAC%' AND justificacion_tec LIKE '%$filtro%' AND bitacora_movimientos.tipo_operacion =  'GenerarRequisicion' 
								AND autorizada = 0 ORDER BY id_requisicion";
				}
			}
