<?php
	...
	**/
	...
	function mostrarDetalle(){
		...
		//Crear la Setencia SQL para seleccionar los datos de la Bitacora de Mtto y del Equipo asociados a la orden de trabajo Seleccionada
		$stm_sql = "SELECT id_bitacora, id_equipo, nom_equipo, familia, turno, horometro, odometro, fecha_mtto FROM equipos JOIN bitacora_mtto ON id_equipo=equipos_id_equipo
					WHERE orden_trabajo_id_orden_trabajo = '$idOrdenTrabajo'";
		...
		if($datos=mysql_fetch_array($rs)){						
			//Obtener la Clave del Vale registrado en el caso que exista
			$idVale = obtenerDato("bd_mantenimiento", "materiales_mtto", "id_vale", "bitacora_mtto_id_bitacora", $datos["id_bitacora"]);
			if($idVale=="")
				$idVale = "N/D";
			echo "								
			<table cellpadding='5'>
	...
	function mostrarDetalleCorr(){
		...
		$stm_sql = "SELECT id_equipo, nom_equipo, familia, turno, horometro, odometro, fecha_mtto 
					FROM (equipos JOIN bitacora_mtto ON id_equipo=equipos_id_equipo) WHERE id_bitacora = '$id_bitacora'";					
		$rs = mysql_query($stm_sql);
		if($datos=mysql_fetch_array($rs)){		
			echo "								
			<table cellpadding='5'>
	...
	/*Funcion que muestra la Bitacora por Equipo*/
	function mostrarBitacoraEquipo(){
		...
		//Crear la consulta cuando el AuxMtto esta registrado
		$stm_sql = "SELECT id_bitacora, equipos_id_equipo, tipo_mtto, fecha_mtto, turno, horometro, odometro, tiempo_total, costo_mtto, num_factura, comentarios
					FROM bitacora_mtto WHERE equipos_id_equipo='$idEquipo' AND fecha_mtto>='$f1' AND fecha_mtto<='$f2' ORDER BY fecha_mtto,tipo_mtto";
		//Ejecutar la consulta
		$rs = mysql_query($stm_sql);
		//Mostrar los resultados obtenidos
		if($datos = mysql_fetch_array($rs)){								
			...
			echo "								
				<table cellpadding='5' width='100%'>
					<caption class='titulo_etiqueta'>REGISTROS EN LA BIT&Aacute;CORA DEL $_POST[txt_fechaIni] AL $_POST[txt_fechaFin] DEL EQUIPO $idEquipo</caption>
					<tr>						
						<td class='nombres_columnas' align='center'>NO. BITACORA</td>
						<td class='nombres_columnas' align='center'>TIPO MTTO.</td>
						<td class='nombres_columnas' align='center'>FECHA REALIZACION</td>
						<td class='nombres_columnas' align='center'>TURNO</td>
						<td class='nombres_columnas' align='center'>HOROMETRO/<br>ODOMETRO</td>
						<td class='nombres_columnas' align='center'>TIEMPO MATENIMIENTO</td>
			...
			do{
				...
				echo "	
					<tr>		
						<td class='nombres_filas' align='center'>$datos[id_bitacora]</td>
						<td class='$nom_clase' align='center'>$datos[tipo_mtto]</td>
						<td class='$nom_clase' align='center'>$fecha</td>
						...
						<td class='$nom_clase' align='center'>$".number_format($datos['costo_mtto'],2,".",",")."</td>
						<td class='$nom_clase' align='center'>$datos[num_factura]</td>
						<td class='$nom_clase' align='center'>$datos[comentarios]</td>";
						?>
						<td class='<?php echo $nom_clase?>'>
							<input type="button" name="btn_verDetalle<?php echo $cont;?>" id="btn_verDetalle<?php echo $cont;?>" class="botones" value="Ver Detalle" onMouseOver="window.estatus='';return true" 
							title="Ver Detalle de la Bit&aacute;cora <?php echo $datos['id_bitacora'];?>" 
							onClick="this.disabled=true;javascript:window.open('verDetalleBitacora.php?id_bitacora=<?php echo $datos['id_bitacora']; ?>&btn=btn_verDetalle<?php echo $cont;?>',
							'_blank','top=50, left=50, width=900, height=700, status=no, menubar=no, resizable=no, scrollbars=yes, toolbar=no, location=no, directories=no');"/>
						</td>
						<?php
				echo "
					</tr>";							
			...
			echo "	</form>
					<tr><td colspan='6'>&nbsp;</td><td class='nombres_columnas'>$".number_format($cant_total,2,".",",")."</td></tr>
			</table>";
		}
		else{
			echo "<br><br><br><br><br><br><br><br><br><br>
			<p class='msje_correcto' align='center'>No Hay Registros en la Bit&aacute;cora del <em>$_POST[txt_fechaIni]</em> AL <em>$_POST[txt_fechaFin]</em> DEL EQUIPO <em>$idEquipo</em></p>";
		}
	}
?>