<?php
	...
	function generarOrdenTrabajo($id_ot){
		...
		if ((isset($_SESSION['datosOT']['metrica'])) && ($cmb_metrica=='HOROMETRO')){		
			...
			estado, autorizo_ot, proveedor_servicio)
			VALUES('$id_ot','$cmb_servicio','$txt_fechaOrdenTrabajo','$txt_fechaProgramada','$cmb_turno',$txt_cantidadMetrica,'0',
			'$txt_operadorEquipo','$txa_comentarios', $estado, '$cmb_autorizoOT','$txt_proveedor')";
		}
		else{
			//Crear la Sentecnia SQL para insertar los datos 
			$stm_sql = "INSERT INTO orden_trabajo (id_orden_trabajo,servicio,fecha_creacion,fecha_prog,turno,horometro,odometro,operador_equipo,comentarios,
			estado, autorizo_ot, proveedor_servicio)
			VALUES('$id_ot','$cmb_servicio','$txt_fechaOrdenTrabajo','$txt_fechaProgramada','$cmb_turno', '0' ,$txt_cantidadMetrica,
			'$txt_operadorEquipo','$txa_comentarios', $estado, '$cmb_autorizoOT','$txt_proveedor')";
		}	
		...
		if($rs){
			...
			registrarOperacion("bd_mantenimiento",$id_ot,"GenerarOT",$_SESSION['usr_reg']);
			//Si se agrega corectamente generar el pdf de la orden de trabajo?>
			<script type='text/javascript' language='javascript'>
				var codigoPopUp = "window.open('../../includes/generadorPDF/ordenTrabajoMtto.php? id=<?php echo $id_ot;  ?>', ";
			...
			$error = mysql_error();
			echo "<br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br>$error";
			//echo "<meta http-equiv='refresh' content='5;url=error.php?err=$error'>";
		}		
	...
	function bitacora($id_ot){
		...
		VALUES ('$id_bitacora', '$id_ot', '$conceptoOT[claveEquipo]', '$tipo_mtto', '$cmb_turno' )";
	...
	function obtenerIdOrdenTrabajo(){
		...
		$stm_sql = "SELECT MAX(id_orden_trabajo) AS cant FROM orden_trabajo WHERE id_orden_trabajo LIKE 'ORT$mes$anio%'";
		$rs = mysql_query($stm_sql);
		if($datos=mysql_fetch_array($rs)){
	...
	function gamasAgregadas($msg_tabla){
		...
		//</caption>";
		echo "<caption class='titulo_etiqueta'>Gamas Agregada a la Orden de Trabajo<u></u>
		<p class='msje_incorrecto'>$msg_tabla</p>
		</caption>";
		...
		echo "      			
			<tr>
        		...
        		<td class='nombres_columnas'>ID DE LA GAMA</td>
        		<td class='nombres_columnas'>NOMBRE DE LA GAMA</td>
      		</tr>";
		...
		foreach ($_SESSION['gamasOT'] as $ind => $gama) {
			echo "<tr>
					<td class='nombres_filas' ><input type='radio' name='rdb_gama' value='$ind' /></td>
