<?php
	...
	function obtenerIdRequisicion(){
		...
		$stm_sql = "SELECT COUNT(id_requisicion) AS cant FROM requisiciones WHERE id_requisicion LIKE 'MAC$mes$anio%'";
	...
	function guardarRequisicion($txa_justificacionReq,$hdn_fecha,$txt_areaSolicitante,$txt_solicitanteReq,$txt_elaboradorReq){
		...
		if($rs){			
			...
			if($band==0){
				...
				registrarOperacion("bd_mantenimiento",$id_requisicion,"GenerarRequisicion",$_SESSION['usr_reg']);
	...
	function borrarFotosSesionMttoConcreto(){
		//Almacenamos el valor de la clave de la requisicion
		$carpeta=$_SESSION["fotosReq"][0]["clave"];
		//Borrar los ficheros temporales
		$h=opendir('mac/documentos/'.$carpeta);
		while ($file=readdir($h)){
			if (substr($file,-4)=='.jpg'){
				@unlink("mac/documentos/".$carpeta."/".$file);
			}
		}
