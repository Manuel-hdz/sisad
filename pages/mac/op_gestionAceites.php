<?php
	...
	**/
	...
	function mostrarAceites(){
		...
		$stm_sql="SELECT nom_aceite,cantidad FROM catalogo_aceites";
	...
	function guardarActualizacionAceite(){
		...
		if(isset($_POST["cmb_aceite"])){
			...
			$sql_stm_ins="UPDATE catalogo_aceites SET cantidad=cantidad+$inc WHERE id_aceite='$idAceite'";
			//Sentencia para ingresar el Registro del incremento a la Bitácora
			$sql_stm_act="INSERT INTO bitacora_aceite (id_bitacora,catalogo_aceites_id_aceite,equipos_id_equipo,fecha,tipo_mov,cantidad,equipo) VALUES ('$idBitacora','$idAceite','','$fecha','E','$inc','N/A')";
		}
		else{
			//Obtener el ID del Aceite
			...
			$sql_stm_ins="INSERT INTO catalogo_aceites (id_aceite,nom_aceite,cantidad) VALUES ('$idAceite','$nomAceite','$cantidad')";
			//Sentencia para ingresar el Registro del incremento a la Bitácora
			$sql_stm_act="INSERT INTO bitacora_aceite (id_bitacora,catalogo_aceites_id_aceite,equipos_id_equipo,fecha,tipo_mov,cantidad,equipo) VALUES ('$idBitacora','$idAceite','','$fecha','I','$cantidad','N/A')";
		}
		//Abrimos la conexion con la Base de datos
		$conn=conecta("bd_mantenimiento");
	...
	function obtenerIdBitacoraAceite(){
		...
		$sql="SELECT MAX(id_bitacora)+1 AS id FROM bitacora_aceite";
	...
	function obtenerIDAceite(){
		...
		$conn=conecta("bd_mantenimiento");
		//Sentencia SQl para extraer el id
		$sql="SELECT MAX(id_aceite)+1 AS id FROM catalogo_aceites";
		//Ejecutar la sentencia
		$rs=mysql_query($sql);
		if($datos=mysql_fetch_array($rs)){
			if($datos["id"]==NULL)
	...
	function mostrarEquipos($fecha,$familia){
		//Abrimos la conexion con la Base de datos
		$conn=conecta("bd_mantenimiento");
		//Extraer los aceites del catalogo
		$rs_aceites = mysql_query("SELECT id_aceite,nom_aceite,cantidad FROM catalogo_aceites WHERE cantidad>0 ORDER BY nom_aceite");
		//Cantidad de Aceites
		...
		//Creamos la sentencia SQL para mostrar los datos del Equipo
		$stm_sql="SELECT id_equipo,nom_equipo FROM equipos WHERE area!='' AND familia='$familia'";		
		//Ejecutamos la sentencia SQL
		$rs=mysql_query($stm_sql);
		//Cantidad de Equipos
		...
		if ($datos=mysql_fetch_array($rs)){
			...
			echo "	<tr>
						...
						<td class='nombres_columnas' align='center' rowspan='2'>ODOMETRO</td>
						<td class='nombres_columnas' align='center' rowspan='2'>ODOMETRO FINAL</td>
					</tr>";
			echo "	<tr>
						<td class='nombres_columnas' align='center'>No.</td>
			...
			do{
				echo "	<tr>					
						...
						<td class="<?php echo $nom_clase;?>">
							<input type="checkbox" name="ckb_equipo<?php echo $cont?>" id="ckb_equipo<?php echo $cont?>" value="<?php echo $datos["id_equipo"];?>" onclick="activarDesactivarRegBitAceite(this,'<?php echo $cont?>');"/>
						</td>
						<?php
				echo "
						...
						<td class="<?php echo $nom_clase;?>">
							<input type="text" class="caja_de_num" id="txt_metrica<?php echo $cont?>" name="txt_metrica<?php echo $cont?>" size="20" maxlength="10" value="" onkeypress="return permite(event,'num',2);" 
							onchange="formatCurrency(value,'txt_metrica<?php echo $cont?>');" readonly="readonly"/>
						</td>
						<td class="<?php echo $nom_clase;?>">
							<input type="text" class="caja_de_num" value="<?php echo obtenerOdometroFinal($datos["id_equipo"]); ?>" readonly="readonly" />
						</td>
						<?php
				echo"	</tr>";			
				//Determinar el color del siguiente renglon a dibujar
	...
	function guardarRegistroAceites(){
		...
		do{
			if(isset($_POST["ckb_equipo$cant"])){
				...
				if($_POST["txt_metrica$cant"] == null)
					$metrica = 0;
				else
					$metrica=str_replace(",","",$_POST["txt_metrica$cant"]);
				//Sentencia para actualizar la cantidad de Aceite en "STOCK"
				$sql_stm_cat="UPDATE catalogo_aceites SET cantidad=cantidad-$cantidad WHERE id_aceite='$aceite'";
				//Sentencia para ingresar el Registro del incremento a la Bitácora
				$sql_stm_bit="INSERT INTO bitacora_aceite (id_bitacora,catalogo_aceites_id_aceite,equipos_id_equipo,fecha,turno,supervisor_mtto,tipo_mov,cantidad,equipo,odometro_horometro) 
								VALUES ('$idBitacora','$aceite','$equipo','$fecha','$turno','$supervisor','S','$cantidad','$equipo','$metrica')";
				//Abrimos la conexion con la Base de datos
				$conn=conecta("bd_mantenimiento");
				//Ejecutar la sentencia de catalogo de aceites
	...
	function guardarRegistroGasolina(){
		
		$idBitacora=obtenerIdBitacoraGasolina();
		$fecha=modFecha($_POST["txt_fecha"],3);
		$equipo=$_POST["txt_equipo"];
		$litros=str_replace(",","",$_POST["txt_litros"]);
		$metrica=str_replace(",","",$_POST["txt_metrica"]);
		$costo=str_replace(",","",$_POST["txt_costo"]);
		$no_vale=strtoupper($_POST["txt_noVale"]);
		$responsable=$_POST["txt_rfc"];
		echo "<br><br><br><br><br><br><br><br><br><br><br><br><br><br><br>";
		
		$sql_stm_bit = "INSERT INTO bitacora_gasolina (id_bitacora_gasolina,equipos_id_equipo,fecha,responsable,cantidad,odometro,costo_litro,no_vale) 
						VALUES ('$idBitacora','$equipo','$fecha','$responsable','$litros','$metrica','$costo','$no_vale')";
		//Abrimos la conexion con la Base de datos
		$conn=conecta("bd_mantenimiento");
		//Ejecutar la sentencia
		$rs=mysql_query($sql_stm_bit);
		...
		if(!$rs){
			?>
				<script>
					setTimeout("alert('Error en registro de bitacora de gasolina')",500);
				</script>
			<?php
			mysql_close($conn);
		}
			...
			?>
				<script>
					setTimeout("alert('Registro de Bitacora <?php echo $idBitacora; ?> Exitoso')",500);
				</script>
			<?php
			mysql_close($conn);
			//Registrar el movimiento en la bitácora de Movimientos
			registrarOperacion("bd_mantenimiento","$idBitacora","RegistroSalidaGasolina",$_SESSION['usr_reg']);
		}
	}//fin de function guardarRegistroGasolina()
	
	//Funcion que calcula el id para la bitacora de Gasolina
	function obtenerIdBitacoraGasolina(){
		...
		$stm_sql = "SELECT MAX(id_bitacora_gasolina) AS cant FROM bitacora_gasolina WHERE id_bitacora_gasolina LIKE 'BITGAS$mes$anio%'";
		$rs = mysql_query($stm_sql);
		if($datos=mysql_fetch_array($rs)){
	...
	function obtenerOdometroFinal($equipo){
		$odometro = 0;
		$conn_fin = conecta("bd_mantenimiento");
		$stm_sql_fin = "SELECT odometro_horometro
						FROM bitacora_aceite
						WHERE equipos_id_equipo LIKE  '$equipo'
						AND tipo_mov =  'S'
						ORDER BY bitacora_aceite.fecha DESC , id_bitacora DESC 
						LIMIT 1";
		$rs_fin = mysql_query($stm_sql_fin);
		if($rs_fin){
			if($datos_fin = mysql_fetch_array($rs_fin))
				$odometro = $datos_fin[0];
		}
		return $odometro;
	}
?>