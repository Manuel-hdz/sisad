<?php
	...
	function mostrarActividadesRealizar($actividadesRealizar){
		...
		echo "<caption class='msje_correcto'><strong>Actividades Agregados a la Orden</strong></caption>";
		echo "      			
			<tr>
				<td width='10%' class='nombres_columnas' align='center'>PARTIDA</td>
				<td width='15%' class='nombres_columnas' align='center'>SISTEMA</td>
        		<td width='15%' class='nombres_columnas' align='center'>APLICACION</td>
			    <td width='40%' class='nombres_columnas' align='center'>ACTIVIDAD</td>
				<td width='10%' class='nombres_columnas' align='center'>FAMILIA</td>
				<td width='10%' class='nombres_columnas' align='center'>EQUIPO</td>
      		</tr>";
		...
		foreach ($actividadesRealizar as $ind => $registro) {
			echo "
			<tr>
				<td align='center' class='nombres_filas'>$registro[partida]</td>
	...
	function generarOrdenTrabajoServicioExterno(){
		...
		$id_otse = obtenerIdOrdenTrabajoSE($area);
		//Realizar la conexion a la BD de mantenimiento
		$conn = conecta("bd_mantenimiento");				
		...
		encargado_compras, solicito, autorizo, depto)
		VALUES('$idOrdenTrabajo', '$fechaRegistro', '$clasificacion', '$fechaSolicitud', '$fechaRecepcion', '$nomProveedor', '$direccion', '$repProveedor', '',
		'$solicito','$autorizo','$nomDepto')";
		...
		if($rs){
			...
			$resultado = 1;
			if(registrarActividades($id_otse)){
				if(isset($_SESSION['materialesUtilizar'])){
					$resultado = registrarMateriales($id_otse);
				}
			}
			...
			if($resultado==1){
				...
				registrarOperacion("bd_mantenimiento",$id_otse,"GenerarOrdenServicioExterno",$_SESSION['usr_reg']);
												
				//Si se agrega corectamente generar el pdf de la orden de trabajo?>
				<script type="text/javascript" language="javascript">															
					var codigoPopUp = "window.open('../../includes/generadorPDF/ordenServicioExterno.php?";
					codigoPopUp += "id_orden=<?php echo $id_otse; ?>&nom_depto=<?php echo $_POST['txt_area']; ?>&fecha_reg=<?php echo $_POST['txt_fechaRegistro']; ?>', ";
				...
				mysql_query("DELETE FROM orden_servicios_externos WHERE id_orden = '$id_otse'");
				mysql_query("DELETE FROM actividades_realizadas WHERE orden_servicios_externos_id_orden = '$id_otse'");
	...
	function registrarActividades($id_otse){
		...
		foreach($_SESSION['actividadesRealizar'] as $ind => $act){
			...
			$stm_sql = "INSERT INTO actividades_realizadas (orden_servicios_externos_id_orden, no_actividad, sistema, aplicacion, descripcion, familia, equipo, costo_actividad)
						VALUES('$id_otse', $noAct, '$act[sistema]', '$act[aplicacion]', '$act[actividad]', '$act[familia]', '$act[claveEquipo]', 0)";
...
";
	...
	function registrarMateriales($id_otse){
		//Variable que nos ayudará a identificar problemas de inserción de datos en la BD
		$band = 0;
		//Recuperar el Id de la Orden de Trabajo de la SESSION
		$idOrdenTrabajo = $_SESSION['ordenServicioExterno']['idOrdenTrabajo'];
		...
		foreach($_SESSION['materialesUtilizar'] as $ind => $regMaterial){
			...
			$stm_sql = "INSERT INTO materiales_usados (orden_servicios_externos_id_orden, descripcion, cantidad)
						VALUES('$id_otse','$regMaterial[material]', $regMaterial[cantidad])";			
	...
	function mostrarOrdenesServiciosExternos(){
		...
		$sql_stm_OTSE .= "fecha_creacion BETWEEN '$fechaIni' AND '$fechaFin' AND id_orden LIKE 'SEC%'";				
		...
		if($datos_OTSE=mysql_fetch_array($rs_OTSE)){
			...
			<table cellpadding='5' width='150%'>
				...
				<tr>
					...
					<td class='nombres_columnas' align='center'>COMPLEMENTAR</td>
					...
					<td class='nombres_columnas' align='center'>ENCARGADO COMPRAS</td>
