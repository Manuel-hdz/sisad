<?php
	...
	**/
	...
	function guardarDatosGama(){												
		...
		$stm_sql = "UPDATE gama SET id_gama='$id_gama', nom_gama='$nom_gama', descripcion='$descripcion', area_aplicacion='$area', familia_aplicacion='$familia', ciclo_servicio=$cicloServ 
	...
	function guardarSistemas(){
			...
			foreach($sistema as $nomApp => $aplicacion){
				...
				foreach($aplicacion as $idAct => $actividad){
					...
					if($rs){						
						...
						$rs_act = mysql_query("INSERT INTO gama_actividades (gama_id_gama,actividades_id_actividad) VALUES('$idGama',$claveActividad)");
		...
		if($band==0){//Redireccionar a la Pagina donde serán agreados los Sistemas a la Gama
			...
			registrarOperacion("bd_mantenimiento",$idGama,"ModificarGama",$_SESSION['usr_reg']);
