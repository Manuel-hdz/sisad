<?php
	...
	function mostrarTituloHorometro(){
		...
		<table cellpadding='5' class='tabla_frm'>      			
			<tr>
				...
				<td width='100' class='nombres_columnas'>&Uacute;LTIMO HOR&Oacute;METRO</td>
	...
	function mostrarEquiposHorometro(){
		...
		if($datos=mysql_fetch_array($rs)){
			...
			$cont = 1;			
			do{			
				//Obtener las Horas Acumuladas y la fecha mayor de los registros por Equipo
				...
				$ultimoHorometro = "No Hay Registro";
				...
				if($datos_reg_final = mysql_fetch_array($rs_reg_final)){
					...
					if($datos_reg_final['fecha']!=NULL){
						...
						$horo_final = mysql_fetch_array(mysql_query("SELECT reg_final FROM horometro_odometro WHERE equipos_id_equipo = '$datos[id_equipo]' AND fecha='$datos_reg_final[fecha]'"));
						$ultimoHorometro = $horo_final["reg_final"];
					}
				}
				...
				<tr>
					...
					<td width='100' class='$nom_clase'>"; ?>
						...
						onchange="formatCurrency(value,'txt_hrsEfectivas<?php echo $cont;?>');"/><?php 
					echo "</td>	
				</tr>";
	...
	function guardarRegHorometro(){
		...
		do{
			...
			if(isset($_POST["txt_horoIni$ctrl"])){
				...
				$stm_sql="INSERT INTO horometro_odometro (equipos_id_equipo,fecha,reg_inicial,reg_final,hrs_efectivas,turno,observaciones)
				VALUES('$ckb_equipo','$fecha','$txt_horoIni','$txt_horoFin','$txt_hrsEfectivas','$cmb_turno','$txa_observaciones')";
				//Ejecutar la sentencia previamente creada
