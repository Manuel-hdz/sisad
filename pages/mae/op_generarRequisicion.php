<?php
	/**
	  * Nombre del Módulo: Mantenimiento Eléctrico                                              
	...
	function obtenerIdRequisicion(){
		...
		$conn = conecta("bd_mantenimientoE");
		...
		$stm_sql = "SELECT COUNT(id_requisicion) AS cant FROM requisiciones WHERE id_requisicion LIKE 'MAE$mes$anio%'";
	...
	function guardarRequisicion($txa_justificacionReq,$hdn_fecha,$txt_areaSolicitante,$txt_solicitanteReq,$txt_elaboradorReq){
		...
		if($rs){			
			...
			foreach ($_SESSION['datosRequisicion'] as $ind => $material) {			
				...
				materiales_id_material,partida)
				VALUES('$_SESSION[id_requisicion]', $material[cantReq], '$material[unidad]', '$material[material]', '$material[aplicacionReq]', '$material[clave]',$cont)";
			...
			if($band==0){
				...
				registrarOperacion("bd_mantenimientoE",$_SESSION['id_requisicion'],"GenerarRequisicion",$_SESSION['usr_reg']);
	...
	function borrarFotosSesionMantenimientoE(){
		//Almacenamos el valor de la clave de la requisicion
		$carpeta=$_SESSION["fotosReq"][0]["clave"];
		//Borrar los ficheros temporales
		$h=opendir('mae/documentos/'.$carpeta);
		while ($file=readdir($h)){
			if (substr($file,-4)=='.jpg'){
				@unlink("mae/documentos/".$carpeta."/".$file);
			}
		}
