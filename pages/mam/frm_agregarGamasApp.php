...
<head>
    ...
    <style type="text/css">
		<!--
		#titulo-agregarAplicacion { position:absolute; left:30px; top:146px; width:306px; height:20px; z-index:11; }
...
<body>
    ...
    <div id="titulo-agregarAplicacion" class="titulo_barra">Editar Sistema Agregado a la Gama</div>
	<?php
	...
	if(isset($_POST['sbt_agregarApp'])){
		//Obtener el nombre de la Aplicacion desde El ComboBox o la Caja de Texto y del Sistema
		if(isset($_POST['cmb_aplicacionSistema'])){
			$nom_app = $_POST['cmb_aplicacionSistema']; 
			$nom_sistema = $_POST['cmb_sistema']; 
		}
		else{
			$nom_app = strtoupper($_POST['txt_nuevaApp']);
			$nom_sistema = ""; 
		}
		
		//Vetrificar que la Aplicacion no haya sido registrada previamente
		if(!isset($_SESSION['sistemasGamaNueva'][$_SESSION['sistemaEditar']][$nom_app])){
			//Realizar la conexion a la BD de Compras
			$conn = conecta("bd_mantenimiento");
			//Cargar las Actividades de la Aplicación Seleccionada en la SESSION
			$_SESSION['sistemasGamaNueva'][$_SESSION['sistemaEditar']][$nom_app] = cargarActividadesApps($nom_app,$nom_sistema);			
			//Cerrar la Conexion con la BD
			mysql_close($conn);			
		}
		else//En el caso de que la Aplicacion ya este registrada, desplegar mensaje
			$msg_tabla = "La Aplicaci&oacute;n $nom_app ya esta agregada al Sistema";
	}
	...
	if(isset($_POST['sbt_eliminarApp']) && isset($_POST['rdb_aplicacion'])){												
		...
		unset($_SESSION['sistemasGamaNueva'][$_SESSION['sistemaEditar']][$_POST['rdb_aplicacion']]);
	...
	if(isset($_POST['sbt_editarApp'])  && isset($_POST['rdb_aplicacion'])){
		...
		echo "<meta http-equiv='refresh' content='0;url=frm_agregarGamasActs.php'>";		
	}?>
	
	
    <fieldset class="borde_seccion" id="agregar-aplicacion" name="agregar-aplicacion">
	<legend class="titulo_etiqueta">Seleccionar las Aplicaciones Correspondiente al Sistema <em><u><?php echo $_SESSION['sistemaEditar'];?></u></em></legend>
	<br />
	<form onsubmit="return valFormAgregarDatoGama(this,'Seleccionar o Ingresar una Aplicación para ser Agregada al Sistema',4,2);" name="frm_agregarAppSistema" method="post" action="frm_agregarGamasApp.php">
	...
	<form onsubmit="return valFormTablaAplicaciones(this);" name="frm_tablaAplicaciones" method="post" action="frm_agregarGamasApp.php">
		<div id="detalle-aplicaciones" class="borde_seccion2" align="center">
			...
			if(count($_SESSION['sistemasGamaNueva'][$_SESSION['sistemaEditar']])!=0){				
		...
		<div id="botones-detApp" align="center">
			...
			<?php if(count($_SESSION['sistemasGamaNueva'][$_SESSION['sistemaEditar']])==0){ ?> disabled="disabled" <?php } ?> onclick="hdn_boton.value='Eliminada'" />
			&nbsp;&nbsp;
			...
			<?php if(count($_SESSION['sistemasGamaNueva'][$_SESSION['sistemaEditar']])==0){ ?> disabled="disabled" <?php } ?> onclick="hdn_boton.value='Editada'" />			
			&nbsp;&nbsp;			
			<input type="button" name="btn_regresar" value="Regresar" class="botones" title="Regresar al Registro de Sistemas" onclick="location.href='frm_agregarGamasSistema.php';" />
			&nbsp;&nbsp;			
