...
<head>
	...
	<script type="text/javascript" language="javascript">
		function consultaMultiple(){
			var cant=prompt("¿Cuantos Artículos se buscarán?","Cantidad...");
			//Comprobar si es un valor numérico 
			if (isNaN(cant))
				//Numero invalido
				alert ("El Dato: '"+cant+"' es Incorrecto, Solo se Aceptan Numeros Enteros");
			else{
				if(cant.indexOf(".")==-1 && cant>0){
					location.href='frm_consultarMateriales2.php?cant='+cant;
				}
				else
					alert ("El Dato: '"+cant+"' es Incorrecto, Solo se Aceptan Numeros Enteros");
			}
...
<body>
	...
	<p class="titulo_etiqueta" align="left">Lista de Materiales</p>	
		<?php 
			//Mostrar la lista de Todos los Materiales
			$res=mostrarMateriales();
		 ?>
	</div>
	...
	<div id="botones" align="center">
		...
		if($res==1){
		?>
		<input type="button" name="btn_conMultiple" title="Consultar varios Art&iacute;culos del Almac&eacute;n" onclick="consultaMultiple();" value="Consulta M&uacute;ltiple" 
		class="botones_largos" onmouseover="window.status='';return true;"/>&nbsp;&nbsp;&nbsp;&nbsp;
		<?php 
		}?>
		<input type="button" name="btn_regresar" title="Regresar al Inicio" onclick="location.href='inicio_mantenimiento.php';" value="Regresar" class="botones" onmouseover="window.status='';return true;"/>
	</div>
