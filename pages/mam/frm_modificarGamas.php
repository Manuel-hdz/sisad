...
<?php
	...
	//Comprobar que el usuario registrado tenga acceso a esta seccion del M�dulo de Mantenimiento
...
<body>		
	...
	if(isset($_GET['btn_guardarGama']) && $_GET['btn_guardarGama']=="GuardarGama"){
		...
		if(!isset($_POST['sbt_modificarGama'])){ 		
			/*Determinar cual usuario esta logeado y en base a ello permitir la Manipulacion de la Informaci�n que le Corresponde*/
			$atributo = "";
			$area = "";
			$estado = 1;//El estado 1 indica que el departamento registrados en la SESSION tiene acceso a la informaci�n de MINA y CONCRETO
			...
			//Cargar las Familias segun el usuario registrado en la SESSION del �rea de MttoMina o MttoConcreto
			if($estado==0){ ?>
				<script type="text/javascript" language="javascript">
					setTimeout("cargarCombo('<?php echo $area;?>','bd_mantenimiento','gama','familia_aplicacion','area_aplicacion','cmb_familiaGama','Familia','')",500);
				</script><?php 
			} ?>
			...
			<table width="100%" cellpadding="5" cellspacing="5" class="tabla_frm">
				...
				<tr>
					...
					<?php 
						...
						<input type="text" name="cmb_areaGama" id="cmb_areaGama" value="MINA" readonly="true" />
					</td>
			...
			//Redireccionar a la Pagina donde ser�n modificados los Sistemas de la Gama
