<?php
	/**
	  * Nombre del Módulo: Recursos Humanos-Mantenimiento
	  * Nombre Programador: Antonio de Jesús Jiménez Cuevas
	  * Fecha: 26/Marzo/2012
	  * Descripción: Este archivo contiene funciones para Consultar un Empleado de la BD de Recursos y asignarle un Rol
	**/
	...
	function mostrarTrabajadores(){
		...
		$stm_sql="SELECT rfc_empleado,CONCAT(nombre,' ',ape_pat,' ',ape_mat) AS nombre,area,puesto FROM empleados WHERE area='MANTENIMIENTO' AND id_empleados_empresa>0 ORDER BY puesto,id_empleados_empresa";
		...
		do{
			if (isset($_POST["ckb_seleccionar".$cont])){
				...
				if (!$rs){
					...
					registrarOperacion("bd_mantenimiento","$rfc","ActualizarRol",$_SESSION['usr_reg']);
				}
			}
			$cont++;
		}while($cont<=$tam);
