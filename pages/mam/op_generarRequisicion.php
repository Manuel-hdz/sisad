<?php
	...
	function obtenerIdRequisicion(){
		...
		$stm_sql = "SELECT COUNT(id_requisicion) AS cant FROM requisiciones WHERE id_requisicion LIKE 'MAM$mes$anio%'";
	...
	function guardarRequisicion($txa_justificacionReq,$hdn_fecha,$txt_areaSolicitante,$txt_solicitanteReq,$txt_elaboradorReq){
		//Verificar que el Id de la requisicion no este ya asignado a una requisicion, si esta en la BD, obtener uno nuevo
	...
	function borrarFotosSesionMttoMina(){
		//Almacenamos el valor de la clave de la requisicion
		$carpeta=$_SESSION["fotosReq"][0]["clave"];
		//Borrar los ficheros temporales
		$h=opendir('mam/documentos/'.$carpeta);
		while ($file=readdir($h)){
			if (substr($file,-4)=='.jpg'){
				@unlink("mam/documentos/".$carpeta."/".$file);
			}
		}
	}//Fin de la funcion borrarFotosSesionMttoMina()
