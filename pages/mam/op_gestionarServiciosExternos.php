<?php
	...
	function generarOrdenTrabajoServicioExterno(){
		$idOrdenTrabajo = obtenerIdOrdenTrabajoSE("MINA");
		$conn = conecta("bd_mantenimiento");	
		...
		if($rs){
			...
			$resultado = 1;
			if(registrarActividades($idOrdenTrabajo)){
				if(isset($_SESSION['materialesUtilizar'])){
					$resultado = registrarMateriales($idOrdenTrabajo);
				}
			}
			...
			if($resultado==1){
				...
				registrarOperacion("bd_mantenimiento",$idOrdenTrabajo,"GenerarOrdenServicioExterno",$_SESSION['usr_reg']);
	...
	function registrarActividades($idOrdenTrabajo){
	...
	function registrarMateriales($idOrdenTrabajo){
		//Variable que nos ayudará a identificar problemas de inserción de datos en la BD
		$band = 0;
		//Recuperar el Id de la Orden de Trabajo de la SESSION
		//$idOrdenTrabajo = $_SESSION['ordenServicioExterno']['idOrdenTrabajo'];
		...
		if($datos_OTSE=mysql_fetch_array($rs_OTSE)){
			...
			<table cellpadding='5' width='150%'>
				...
				<tr>
					...
					<td class='nombres_columnas'>COMPLEMENTAR</td>
