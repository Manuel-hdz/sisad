<?php
	...
	**/
	...
	function obtenerIdNomina(){
		...
		$stm_sql_nom = "SELECT COUNT(id_nomina) AS cant FROM nominas WHERE id_nomina LIKE 'NOMMAM$mes$anio%'";
		...
		if($num > 0) {
			...
			if(isset($_POST["btn_continuar"])){
				$stm_sql = "INSERT INTO  nominas ( id_nomina, periodo, fecha_registro, fecha_inicio, fecha_fin, area, anio, mes, finalizada)
							VALUES ( '$id_nomina',  'SEMANA',  '$fechaReg',  '$fechaI',  '$fechaF',  'MANTENIMIENTO MINA',  '$anio',  '$mes', 1)";
			}
			else if(isset($_POST["btn_avance"])){
				$stm_sql = "INSERT INTO  nominas ( id_nomina, periodo, fecha_registro, fecha_inicio, fecha_fin, area, anio, mes, finalizada)
							VALUES ( '$id_nomina',  'SEMANA',  '$fechaReg',  '$fechaI',  '$fechaF',  'MANTENIMIENTO MINA',  '$anio',  '$mes', 0)";
			}
