	...
	function generarReporte($tipo_rpt){
		...
		switch($tipo_rpt){
			case 1://Reporte por Area
				...
				$stm_sql ="SELECT id_bitacora, area,familia,id_equipo,fecha_mtto,tipo_mtto,horometro,odometro,costo_mtto 
							FROM equipos JOIN bitacora_mtto ON id_equipo=bitacora_mtto.equipos_id_equipo 
						   	WHERE area='$_POST[cmb_area]' AND tipo_mtto='CORRECTIVO' AND fecha_mtto>='$f1' AND fecha_mtto<='$f2' ORDER BY fecha_mtto";
				
					//Mensaje que desplegara el titulo de la tabla
				$msg_titulo = "Reporte de Mantenimientos Correctivos del &Aacute;rea de <em><u>$_POST[cmb_area]</u></em>&nbsp;
								<br>En el Periodo del <em><u>$_POST[txt_fechaIni]</u></em> al <em><u>$_POST[txt_fechaFin]</u></em>";
				
				if($_POST['cmb_area']=="CONCRETO")
					$msg_grafica = "Mantenimientos Correctivos del Área de Concreto";
				else
					$msg_grafica = "Mantenimientos Correctivos del Área de Mina";
				
				//Crear el Mensaje en caso de que la consulta no arroje ningún resultado
				$msg_error = "<label class='msje_correcto' align='center'>No se Encontr&oacute; Ning&uacute;n Resultado de Mantenimientos Correctivos del &Aacute;rea <em><u>
				$_POST[cmb_area]</u></em>&nbsp;
								<br>En las Fechas del <em><u>$_POST[txt_fechaIni]</u></em> al <em><u>$_POST[txt_fechaFin]</u></em></label>";	
						
				//Definir datos del reporte de mantenimientos correctivo en la SESSION
				$datosCorrectivos = array("cmb_area"=>$_POST['cmb_area'],"txt_fechaIni"=>$_POST['txt_fechaIni'],"txt_fechaFin"=>$_POST['txt_fechaFin']);
				$_SESSION['datosRptCorrectivos'] = $datosCorrectivos;
			break;
			...
			case 2://Reporte por Familia
				...
				$stm_sql ="SELECT id_bitacora,area,familia,id_equipo,fecha_mtto,tipo_mtto,horometro,odometro,costo_mtto 
							FROM equipos JOIN bitacora_mtto ON id_equipo=bitacora_mtto.equipos_id_equipo 
						   	WHERE familia='$_POST[cmb_familia]' AND area='$area' AND tipo_mtto='CORRECTIVO' AND fecha_mtto>='$f3' AND fecha_mtto<='$f4' ORDER BY fecha_mtto";
				...
				//Crear el Mensaje en caso de que la consulta no arroje ningún resultado
				$msg_error = "<label class='msje_correcto' align='center'>No se Encontr&oacute; Ning&uacute;n Resultado de Mantenimientos Correctivos de la Familia <em><u>
				$_POST[cmb_familia]</u></em>&nbsp;
								<br>En las Fechas del <em><u>$_POST[txt_fechaIni]</u></em> al <em><u>$_POST[txt_fechaFin]</u></em></label>";	
						
				//Definir datos del reporte de mantenimientos correctivos en la SESSION
				$datosCorrectivos = array("cmb_familia"=>$_POST['cmb_familia'],"hdn_area"=>$_POST['hdn_area'],"txt_fechaIni"=>$_POST['txt_fechaIni'],
			...
			case 3://Reporte por Equipo																														 
				...
				$stm_sql ="SELECT id_bitacora, area,familia,id_equipo,fecha_mtto,tipo_mtto,horometro,odometro,costo_mtto 
							FROM equipos JOIN bitacora_mtto ON id_equipo=bitacora_mtto.equipos_id_equipo 
						   	WHERE tipo_mtto='CORRECTIVO' AND id_equipo='$id_equipo' AND fecha_mtto!='0000-00-00' ORDER BY fecha_mtto";
				
				//Mensaje que desplegara el titulo de la tabla				
				$msg_titulo = "	Reporte de Mantenimientos Correctivos Correspondiente al Equipo: <em><u>$id_equipo</u></em>";
				$msg_grafica = "Mantenimientos Correctivos del Equipo: $id_equipo";
				
				//Crear el Mensaje en caso de que la consulta no arroje ningún resultado
				$msg_error = "<label class='msje_correcto' align='center'>No se Encontr&oacute; Ning&uacute;n Resultado de Mantenimientos Correctivos del Equipo 
							  <em><u>$id_equipo</u></em>";	
						
				//Definir datos del reporte de mantenimientos correctivos en la SESSION
				$datosCorrectivos = array("cmb_familia"=>$_POST['cmb_familia'],"cmb_equipo"=>$_POST['cmb_equipo']);
				$_SESSION['datosRptCorrectivos'] = $datosCorrectivos;
			break;
			...
			case 4://Reporte Por Costos				
				...
				$stm_sql ="SELECT id_bitacora,area,familia,id_equipo,fecha_mtto,tipo_mtto,horometro,odometro,costo_mtto 
							FROM equipos JOIN bitacora_mtto ON id_equipo=bitacora_mtto.equipos_id_equipo
						   	WHERE tipo_mtto='CORRECTIVO' AND costo_mtto>=($costo_menor-0.01) AND costo_mtto<=($costo_mayor+0.01) AND fecha_mtto!='0000-00-00' ".$paramExtra." 
							ORDER BY id_bitacora"; 
						
				//Mensaje para desplegar en el titulo de la tabla
				$msg_titulo = "Reporte de Mantenimientos Correctivos Correspondiente al Rango de <em><u>$ $_POST[txt_nivelInf]</u></em> y <em><u>$ $_POST[txt_nivelSup]</u></em>";
				$msg_grafica = "Mantenimientos Correctivos entre $ $costo_menor y $ $costo_mayor";
				...
				$datosCorrectivos = array("txt_nivelInf"=>$_POST['txt_nivelInf'],"txt_nivelSup"=>$_POST['txt_nivelSup']);
				$_SESSION['datosRptCorrectivos'] = $datosCorrectivos;
			break;	
		}			
		...
		if($datos = mysql_fetch_array($rs)){		
			...
			echo "	<td class='nombres_columnas'>&Aacute;REA</td>
				...
				<form name='frm_mostrarDetalleRC' method='post' action='frm_reporteCorrectivo.php'>
			...
			do{
				...
				//if($tipo_rpt==4){
					...
					<td class='nombres_filas'>
						<input type='checkbox' name='RC$cont' value='$datos[id_bitacora]' onClick='javascript:document.frm_mostrarDetalleRC.submit();'/>						
					</td>
		...
		<table width="100%">
			<tr>
				<td align="center">
					...
					onclick="location.href='frm_reporteCorrectivo.php'" />
				</td>
				<?php if($flag==1) { ?>              
					<td align="center">
						<form action="guardar_reporte.php" method="post">
							<input name="hdn_consulta" type="hidden" value="<?php echo $stm_sql; ?>" />
							<input name="hdn_nomReporte" type="hidden" value="Consulta Mtto Correctivo " />                  		
							<input name="hdn_origen" type="hidden" value="mttoCorrectivo" />
				...
				if($flag==1){ ?>
					<td align="center">                  
						<?php 
						$datosGrapCorrectivos = array("hdn_consulta"=>$stm_sql, "hdn_msg"=>$msg_grafica);
						$_SESSION['datosGrapCorrectivos'] = $datosGrapCorrectivos;
						?>						
						<input type="button" name="btn_verGrafica" class="botones" value="Ver Grafica" title="Ver Gr&aacute;fica de Mantenimientos Correctivos" 
						onClick="javascript:window.open('verGraficas.php?graph=mttoCorrectivo',
	...
	function mostrarDetalleRC($clave,$no_reporte){
		...
		<div id="btns-rpt">
			<table align="center">
				<tr>
					<td>
					<form action="frm_reporteCorrectivo.php" method="post">
						<input name="hdn_tipoRpt" type="hidden" value="<?php echo $no_reporte; ?>" />
						<input name="sbt_regresar" type="submit" class="botones" value="Regresar" title="Regresar a la Pantalla del Reporte Mantenimientos Correctivos" 
