...
<head>
	...
	<script type="text/javascript" src="includes/ajax/registroTiempoXActs.js"></script>
    <style type="text/css">
		<!--
		#titulo-agregarActividad { position:absolute; left:30px; top:146px; width:306px; height:20px; z-index:11; }
...
<body>
    ...
    <div id="titulo-agregarActividad" class="titulo_barra">Editar Aplicaci&oacute;n Agregada al Sistema </div><?php
	...
	if(isset($_POST['sbt_agregarAct'])){
		...
		if(!in_array($nom_act,$_SESSION['sistemasGamaNueva'][$_SESSION['sistemaEditar']][$_SESSION['appEditar']]))
			$_SESSION['sistemasGamaNueva'][$_SESSION['sistemaEditar']][$_SESSION['appEditar']][] = $nom_act;//Guardar de forma individual cada Actividad dentro de la Aplicación
		else
			$msg_tabla = "La Actividad $nom_act ya esta agregada a la Aplicaci&oacute;n";//En el caso de que la Actividad ya este registrada, desplegar mensaje
	}
	
	
	//Eliminar una Actividad del Registro
	if(isset($_POST['sbt_eliminarAct']) && isset($_POST['rdb_actividad'])){												
		$nom_act = $_SESSION['sistemasGamaNueva'][$_SESSION['sistemaEditar']][$_SESSION['appEditar']][$_POST['rdb_actividad']];
		//Eliminar una Aplicacion del registro del Sistema
		unset($_SESSION['sistemasGamaNueva'][$_SESSION['sistemaEditar']][$_SESSION['appEditar']][$_POST['rdb_actividad']]);
		
		//Desplegar mensaje cuando se elimina una Actividad del Registro
		$msg_tabla = "La Actividad $nom_act fue eliminada de la Aplicaci&oacute;n";				
	}?>
	...
	<form onsubmit="return valFormAgregarDatoGama(this,'Seleccionar o Ingresar una Actividad para ser Agregada a la Aplicación',4,3);" name="frm_agregarAppSistema" method="post" action="frm_agregarGamasActs.php">
	<table width="100%" cellpadding="5" cellspacing="5" class="tabla_frm">		
		<tr>
			...
			<td  width="10%"><div align="right">Aplicaci&oacute;n</div></td>
			<td  width="25%">
			...
			<td  width="35%" align="center">
			  <input name="ckb_nuevaAct" type="checkbox" id="ckb_nuevaAct" onclick="activarCkbNuevo(this,'cmb_actividadesApp','txa_nuevaAct');"  />
			  Nueva Actividad
		      <textarea name="txa_nuevaAct" id="txa_nuevaAct" onkeyup="return ismaxlength(this);" class="caja_de_texto" rows="2" cols="30"
                onkeypress="return permite(event,'num_car', 0);" disabled="disabled" ></textarea>
			</td>
		</tr>
		<tr>
	...
	<form onsubmit="return valFormTablaActividades(this);" name="frm_tablaActividades" method="post" action="frm_agregarGamasActs.php">
		<div id="detalle-actividades" class="borde_seccion2" align="center">
			...
			if(count($_SESSION['sistemasGamaNueva'][$_SESSION['sistemaEditar']][$_SESSION['appEditar']])!=0){				
		...
		<div id="botones-detAct" align="center">
			...
			<?php if(count($_SESSION['sistemasGamaNueva'][$_SESSION['sistemaEditar']][$_SESSION['appEditar']])==0){ ?> disabled="disabled" <?php } ?> />						
			&nbsp;&nbsp;			
			<input type="button" name="btn_regresar" value="Regresar" class="botones" title="Regresar al Registro de Aplicaciones" onclick="location.href='frm_agregarGamasApp.php';" />
			&nbsp;&nbsp;			
