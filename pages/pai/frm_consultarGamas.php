	...
	if(!isset($_POST['sbt_consultarGama']) && !isset($_POST['sbt_gamasRelacionadas'])){ 
		...
		if($estado==0){ ?>		
			<script type="text/javascript" language="javascript">
				setTimeout("cargarCombo('<?php echo $area;?>','bd_paileria','gama','familia_aplicacion','area_aplicacion','cmb_familiaGama','Familia','<?php echo $familia; ?>')",500);
			</script><?php 
		...
		window.onload = function(){ 
			...
			if($familia!=""){?>
				setTimeout("cargarComboEspecifico('<?php echo $familia; ?>','bd_paileria','gama','id_gama','familia_aplicacion','area_aplicacion','<?php echo $area; ?>','cmb_claveGama','Clave','<?php echo $claveGama; ?>');",500);
			<?php } ?>
		}
		</script>
		...
		<table width="100%" cellpadding="5" cellspacing="5" class="tabla_frm">
			<tr>
				...
				<td width="30%">
					<?php if($estado==1){ ?>
						...
						onchange="cargarCombo(this.value,'bd_paileria','gama','familia_aplicacion','area_aplicacion','cmb_familiaGama','Familia','');">
					...
					<?php } else { ?>
						<select name="cmb_areaGama" class="combo_box" 
						onchange="cargarCombo(this.value,'bd_paileria','gama','familia_aplicacion','area_aplicacion','cmb_familiaGama','Familia','');" <?php echo $atributo;?>>
							<option value="">&Aacute;rea</option>						
							<option value="CONCRETO" <?php if($area=="CONCRETO") echo "selected='selected'"; ?>>CONCRETO</option>
							<option value="MINA" <?php if($area=="MINA") echo "selected='selected'"; ?>>MINA</option>
							<option value="GOMAR" <?php if($area=="GOMAR") echo "selected='selected'"; ?>>GOMAR</option>
			...
			<tr>
				<td><div align="right">Duraci&oacute;n Aproximada:</div></td>
				<td><?php 
					$txtDuracion = "";
					if(isset($_POST['cmb_claveGama']) && $_POST['cmb_claveGama']!=""){
						$conn=conecta("bd_paileria");
						$sql="SELECT tiempo_aprox FROM gama_actividades WHERE gama_id_gama='$_POST[cmb_claveGama]'";
						$rs=mysql_query($sql);
						if($datos=mysql_fetch_array($rs)){
							$horas=0;
							$minutos=0;
							do{
								$horas+=intval(substr($datos["tiempo_aprox"],0,2));
								$minutos+=intval(substr($datos["tiempo_aprox"],3,2));
							...
							if($horas>0)
								$txtDuracion = $horas." hrs ".$minutos." min";
							else
								$txtDuracion = $minutos." min";
						}
					}?>
				<input type="text" name="txt_durAprox" id="txt_durAprox" readonly="readonly" value="<?php echo $txtDuracion?>" size="15"/></td>				
			</tr>
			<tr>
				<td colspan="4">&nbsp;</td>
			</tr>
