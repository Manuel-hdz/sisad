<?php
	...
	**/
	...
	function guardarDatosGama(){						
		...
		$color = $_SESSION['datosGamaNueva']['color'];
		
		
		//Crear la Sentecnia SQL para insertar los datos de la Gama
		$stm_sql = "INSERT INTO gama (id_gama,nom_gama,descripcion,area_aplicacion,familia_aplicacion,ciclo_servicio,color) VALUES('$id_gama','$nom_gama','$descripcion','$area','$familia',$cicloServ,'$color')";
	...
	function guardarSistemas($idGama){
		...
		foreach($_SESSION['sistemasGamaNueva'] as $nomSistema => $sistema){
			...
			foreach($sistema as $nomApp => $aplicacion){
				...
				foreach($aplicacion as $idAct => $actividad){
					//Si el indice es mayor a 0, calcular los negativos de cada cual, para registrar sus tiempos
					if($idAct>0)
					...
					if($idAct>=0){
						//Variable para controlar las actividades que tiene el indice -0
						$temp=" $idAct ";
						if($temp!=" -0 "){
							...
							if($rs){						
								...
								$rs_act = mysql_query("INSERT INTO gama_actividades (gama_id_gama,actividades_id_actividad,tiempo_aprox) VALUES('$idGama',$idActividad,'$tiempoAprox')");
		...
		if($band==0){//Redireccionar a la Pagina donde serán agreados los Sistemas a la Gama
			...
			registrarOperacion("bd_paileria",$idGama,"AgregarGamaNueva",$_SESSION['usr_reg']);
	...
	function verActividadesApp($msg_tabla){
		...
		foreach ($_SESSION['sistemasGamaNueva'][$_SESSION['sistemaEditar']][$_SESSION['appEditar']] as $ind => $actividad) {
			...
			if($ind>0)
				$indiceAuxiliar=$ind*-1;
			//En el caso del 0, dejar -0 como cadena
			else
				$indiceAuxiliar="-0";
			if(isset($_SESSION["sistemasGamaNueva"][$_SESSION['sistemaEditar']][$_SESSION['appEditar']][$indiceAuxiliar]))
				$actTiempo=$_SESSION["sistemasGamaNueva"][$_SESSION['sistemaEditar']][$_SESSION['appEditar']][$indiceAuxiliar];
			else
				$actTiempo="00:00";
			//Verificar que solo se muestren las actividades
			if($ind>=0){
				$temp=" $ind ";
				if($temp!=" -0 "){
					echo "
						<tr>
							...
							<img src="../../images/editar.png" width="25" height="25" onclick="guardarTiempoXActividad(txt_tiempoAprox<?php echo $ind?>,'<?php echo $ind?>')" style="cursor:pointer"/>
							<?php
					echo "</td>
						</tr>";			
					//Determinar el color del siguiente renglon a dibujar
