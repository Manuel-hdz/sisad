<?php
	function mostrarRequisiciones(){
		...
		$conec=conecta("bd_paileria");
		?>
		<table cellpadding='5' cellspacing='5' class='tabla_frm' width='100%'>
			<tr>
				<td colspan='4' align='center' class='titulo_etiqueta'>Requisiciones Publicadas por GOMAR entre el <?php echo modFecha($fechaI,2)." y el ".modFecha($fechaF,2); ?></td>
			</tr>
			<tr>
				<td colspan='4'>&nbsp;</td>
	      	</tr>
			<tr align='center'>
				<td class='nombres_columnas_gomar'>N&Uacute;MERO</td>
	        	<td class='nombres_columnas_gomar'>FECHA DE PUBLICACI&Oacute;N</td>
				<td class='nombres_columnas_gomar'>ESTADO</td>
				<td class='nombres_columnas_gomar'>PRIORIDAD</td>
				<td class='nombres_columnas_gomar'>SELECCIONAR</td>
			</tr>
			<?php
			if($filtro==""){
			...
			if($row = mysql_fetch_array($rs)){
				...
				do{
					...
					?>
					<tr>
						<td class='nombres_filas_gomar' align='center'>
		...
		if($datos=mysql_fetch_array($rs)){
			...
			<table cellpadding='5' width='100%'>
				...
				<tr>
        			...
        			<td class='nombres_columnas_gomar' align='center'>UNIDAD</td>
	...
	function autorizarRequisicion($id_req){
		...
		if($rs){
			...
			<?php
			registrarOperacion("bd_paileria",$id_req,"AutorizarRequisicion",$_SESSION['usr_reg']);
		} else {
			?>
			<script>
