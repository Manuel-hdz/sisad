<?php
	...
	/*	$sql_stm = "SELECT id_orden_trabajo, servicio, area, familia, id_equipo FROM (equipos JOIN bitacora_mtto ON id_equipo = equipos_id_equipo) 
		...
		*/
		...
		$sql_stm = "SELECT id_orden_trabajo, equipos_id_equipo, servicio, fecha_creacion, fecha_prog, orden_trabajo.turno, orden_trabajo.horometro, orden_trabajo.odometro, operador_equipo, orden_trabajo.comentarios, orden_trabajo.estado, autorizo_ot
					FROM (
						orden_trabajo
						JOIN bitacora_mtto ON id_orden_trabajo = bitacora_mtto.orden_trabajo_id_orden_trabajo
					)
					JOIN actividades_ot ON actividades_ot.orden_trabajo_id_orden_trabajo = id_orden_trabajo
					JOIN gama ON id_gama = gama_id_gama
					WHERE servicio =  '$servicio'
					AND area_aplicacion =  '$area'
					AND  `familia_aplicacion` =  '$familia'
					AND fecha_creacion >=  '$f1'
					AND fecha_creacion <=  '$f2'
			...
			<table cellpadding='7' width='1500'>      			
				...
				<tr>
					<td class='nombres_columnas_gomar'>DETALLE</td>
						<td class='nombres_columnas_gomar'>ID ORDEN TRABAJO</td>
						<td class='nombres_columnas_gomar'>CLAVE EQUIPO</td>
						<!-- <td class='nombres_columnas_gomar' width='70'>ESTADO</td> -->
						<td class='nombres_columnas_gomar' width='80'>SERVICIO</td>
						<td class='nombres_columnas_gomar'>FECHA CREACION</td>						
						<td class='nombres_columnas_gomar'>FECHA PROG.</td>						
						<td class='nombres_columnas_gomar' width='150'>TURNO</td>						
						<td class='nombres_columnas_gomar'>HOROMETRO/ODOMETRO</td>
						<td class='nombres_columnas_gomar' width='90'>OPERADOR</td>						
						<td class='nombres_columnas_gomar'>COMENTARIOS</td>	
						<td class='nombres_columnas_gomar' width='90'>ESTADO</td>						
						<td class='nombres_columnas_gomar'>AUTORIZACION</td>											
				</tr>				
	...
	function mostrarDetalleOT($ckb){
		...
		if($datos=mysql_fetch_array($rs)){
			...
			<table cellpadding='5'>
				...
				<tr>
					<td class='nombres_columnas_gomar'>GAMA</td>
					<td class='nombres_columnas_gomar'>NOMBRE</td>						
					<td class='nombres_columnas_gomar'>DESCRIPCION</td>
					<td class='nombres_columnas_gomar'>AREA</td>
					<td class='nombres_columnas_gomar'>FAMILIA</td>
					<td class='nombres_columnas_gomar'>CICLO DE SERVICIO</td>
				</tr>";
			$nom_clase = "renglon_gris";
			$cont = 1;	
			do{
				...
				echo "<tr>		
						<td class='nombres_filas_gomar'>$datos[gama_id_gama]</td>	
		...
		<div id="btn-detalleOT" align="center">
		<form name="" action="frm_consultarOrdenTrabajo.php" method="post">
			...
			<input name="btn_verPDF" type="button" class="botones" value="Ver PDF" title="Ver Archivo PDF de la Requisici&oacute;n Seleccionada" onmouseover="window.status='';return true" 
			onclick="window.open('../../includes/generadorPDF/ordenTrabajoMttoMina.php?id=<?php echo $ckb;  ?>','_blank','top=100, left=100, width=1035, height=723, status=no, menubar=yes, resizable=yes, scrollbars=yes, toolbar=no, location=no, directories=no');"/>
			&nbsp;&nbsp;&nbsp;&nbsp;
