<?php
	...
	function obtenerIdRequisicion(){
		...
		$stm_sql = "SELECT COUNT(id_requisicion) AS cant FROM requisiciones WHERE id_requisicion LIKE 'PAI$mes$anio%'";
	...
	function guardarRequisicion($txa_justificacionReq,$hdn_fecha,$txt_areaSolicitante,$txt_solicitanteReq,$txt_elaboradorReq){
		...
		if($rs){			
			...
			if($band==0){
				...
				registrarOperacion("bd_paileria",$_SESSION['id_requisicion'],"GenerarRequisicion",$_SESSION['usr_reg']);
	...
	function mostrarRegistros($datosRequisicion){
		...
		<table cellpadding='5' align='center'>      			
			<tr>
				<td width='80' class='nombres_columnas_gomar' align='center'>CLAVE</td>
        		<td width='180' class='nombres_columnas_gomar' align='center'>NOMBRE (DESCRIPCI&Oacute;N)</td>
				<td width='100' class='nombres_columnas_gomar' align='center'>UNIDAD DE MEDIDA</td>
			    <td width='70' class='nombres_columnas_gomar' align='center'>CANT.</td>
				<td width='120' class='nombres_columnas_gomar' align='center'>APLICACI&Oacute;N</td>
				<td width='120' class='nombres_columnas_gomar' align='center'>CENTRO DE COSTOS</td>
				<td width='120' class='nombres_columnas_gomar' align='center'>CUENTA</td>
				<td width='120' class='nombres_columnas_gomar' align='center'>SUBCUENTA</td>
				<td width='30' class='nombres_columnas_gomar'></td>
      		</tr>";
		...
		foreach ($datosRequisicion as $ind => $material) {
			...
			foreach ($material as $key => $value) {
				switch($key){
					case "clave":
						echo "<td class='nombres_filas_gomar' align='center'>$value</td>";
						$claveMat=$value;
					break;
					case "material":
	...
	function borrarFotosSesionPaileria(){
		//Almacenamos el valor de la clave de la requisicion
		$carpeta=$_SESSION["fotosReq"][0]["clave"];
		//Borrar los ficheros temporales
		$h=opendir('pai/documentos/'.$carpeta);
		while ($file=readdir($h)){
			if (substr($file,-4)=='.jpg'){
				@unlink("pai/documentos/".$carpeta."/".$file);
			}
		}
