...
<body>
	<div id="parrilla-menu1" align="center">
		...
		if(isset($_GET['err'])) { 
			if($err=="AccesoNegado") {
				$usuarios=verificarUsuarioPermisos("Produccion");
				$band=0;
				...
				//Verificar con el administrador de Gerencia Tecnica
				if($band==0){
					$conn=conecta("bd_usuarios");
					$sql="SELECT usuario FROM usuarios WHERE depto='GerenciaTecnica' AND tipo_usuario='administrador'";
					$rs=mysql_query($sql);
					if($datos=mysql_fetch_array($rs)){
						do{
							if($usr_reg==$datos["usuario"])
								$band=1;
				...
				if($band==1){?>
					<meta http-equiv="refresh" content="7;url=inicio_produccion.php">
					<p>
