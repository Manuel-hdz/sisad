<?php
	function mostrarRequisiciones(){
		...
		$conec=conecta("bd_produccion");
		?>
		<table cellpadding='5' cellspacing='5' class='tabla_frm' width='100%'>
			<tr>
				<td colspan='4' align='center' class='titulo_etiqueta'>Requisiciones Publicadas por PRODUCCION entre el <?php echo modFecha($fechaI,2)." y el ".modFecha($fechaF,2); ?></td>
			</tr>
			<tr>
				<td colspan='4'>&nbsp;</td>
	      	</tr>
	...
	function autorizarRequisicion($id_req){
		...
		if($rs){
			...
			<?php
			registrarOperacion("bd_produccion",$id_req,"AutorizarRequisicion",$_SESSION['usr_reg']);
		} else {
			?>
			<script>
