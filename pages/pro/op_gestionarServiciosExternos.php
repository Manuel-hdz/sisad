<?php
	...
	function obtenerIdOrdenTrabajoSE($area){
		...
		$prefijo = "";
		if($area=="CONCRETO" || $area=="PRODUCCION")
			$prefijo = "SEC";
		else if($area=="MINA")
			$prefijo = "SEM";
	...
	function generarOrdenTrabajoServicioExterno(){
		...
		if($rs){
			...
			if($resultado==1){
				...
				registrarOperacion("bd_produccion",$idOrdenTrabajo,"GenerarOrdenServicioExterno",$_SESSION['usr_reg']);
				...
				//Cerrar la Conexion con la BD
				$error = "Error al Tratar de Agregar los Datos para la Orden de Trabajo para Servicios Externos";
		...
		if($datos_OTSE=mysql_fetch_array($rs_OTSE)){
			...
			<table cellpadding='5' width='150%'>
				...
				<tr>
					...
					<td class='nombres_columnas'>ENCARGADO COMPRAS</td>
