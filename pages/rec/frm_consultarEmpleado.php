...
<title>Sistema de Gesti&oacute;n Empresarial, Producci&oacute;n y Operaci&oacute;n</title>
    ...
    <style type="text/css">
		...
		#consultar-empleado-baja {position:absolute; left:30px; top:500px; width:592px; height:150px; z-index:11;}
		#consultar-empleado-baja2 {position:absolute; left:680px; top:500px; width:280px; height:150px; z-index:13; }
		#res-spider {position:absolute;z-index:15;}
		#res-spider2 {position:absolute;z-index:15;}
...
<body>
	...
	<fieldset class="borde_seccion" id="consultar-empleado-baja">
		<legend class="titulo_etiqueta">Consultar Trabajador Baja por Nombre</legend>	
		<br>		
		<form onSubmit="return valFormconsultarEmpleado1(this);" name="frm_consultarEmpleadoBaja1" method="post" action="frm_consultarEmpleado.php">
			<table width="587" border="0" cellpadding="5" cellspacing="5" class="tabla_frm">
				<tr>
				  <td width="90"><div align="right">&Aacute;rea</div></td>
					<td><?php 
						...
						//Confirmar que la consulta fue exitosa y guardar los datos en el ComboBox		
						if($datos = mysql_fetch_array($rs)){?>
							<select name="cmb_area_baja" id="cmb_area_baja" class="combo_box" onchange="txt_nombre_baja.value='';lookup(txt_nombre_baja,'empleados_baja',cmb_area_baja.value,'2');">				
							<?php //Colocar el mensaje en definido en el parametro $msj para ser desplegado en el ComboBox
							echo "<option value='todo'>Seleccionar</option>";
							do{
								echo "<option value='$datos[area]'>$datos[area]</option>";
				...
				<tr valign="top">
				  <td width="90"><div align="right">Trabajador</div></td>
					<td width="462">
						<input type="text" name="txt_nombre_baja" id="txt_nombre_baja" onkeyup="lookup(this,'empleados_baja',cmb_area_baja.value,'2');" 
						value="" size="60" maxlength="80" onkeypress="return permite(event,'car',0);"/>
						<div id="res-spider">
				...
				<tr>
					<td align="center" colspan="2">
						...
						title="Consultar Informaci&oacute;n del Empleado Seleccionado"  onmouseover="window.status='';return true" value="Consultar" />
                        &nbsp;&nbsp;&nbsp;
						<input name="btn_cancelar_baja" type="button" value="Cancelar" class="botones" title="Regresar al Men&uacute; de Empleados"
						onclick="location.href='menu_empleados.php'" />&nbsp;&nbsp;&nbsp;
						<input type="reset" class="botones" value="Reestablecer" title="Borra los criterios de b&uacute;squeda y reestablece el formulario"/>
					</td>
                </tr>
	...
	<fieldset id="consultar-empleado-baja2" class="borde_seccion">
		<legend class="titulo_etiqueta">Consultar Todos los Trabajadores Baja</legend>	
		<br>
		<form name="frm_consultarEmpleadoBaja2" method="post" action="frm_consultarEmpleado.php">
			<table align="center" border="0" cellpadding="5" cellspacing="5" class="tabla_frm">
				<tr><td>&nbsp;</td></tr>
				<tr>
    	       		<td>
                		<input name="sbt_todos_baja" type="submit" value="Consultar" class="botones" title="Consultar todos los Empleados Baja"
                    	onmouseover="window.status='';return true"/>
               		</td>
				</tr>
				<tr><td>&nbsp;</td></tr>
		...
		<div align="center" id="botones">
			...
			$ruta = "";
			if ($res[1] == 1) {
				$ruta = "../../includes/generadorPDF/fotos_empleados.php?opcion=$res[1]&nombre=$_POST[txt_nombre]";
			}
			else if ($res[1] == 2 || $res[1] == 5) {
				$ruta = "../../includes/generadorPDF/fotos_empleados.php?opcion=$res[1]";
			}
			else if ($res[1] == 3) {
				$ruta = "../../includes/generadorPDF/fotos_empleados.php?opcion=$res[1]&area=$_POST[cmb_area]";
			}
			else if ($res[1] == 4) {
				$ruta = "../../includes/generadorPDF/fotos_empleados.php?opcion=$res[1]&nombre=$_POST[txt_nombre_baja]";
			}

			$ruta_codBarras = "";
			if ($res[1] == 1) {
				$ruta_codBarras = "../../includes/generadorPDF/codigo_barras_empleados.php?opcion=$res[1]&nombre=$_POST[txt_nombre]";
			}
			else if ($res[1] == 2 || $res[1] == 5) {
				$ruta_codBarras = "../../includes/generadorPDF/codigo_barras_empleados.php?opcion=$res[1]";
			}
			else if ($res[1] == 3) {
				$ruta_codBarras = "../../includes/generadorPDF/codigo_barras_empleados.php?opcion=$res[1]&area=$_POST[cmb_area]";
			}
			else if ($res[1] == 4) {
				$ruta_codBarras = "../../includes/generadorPDF/codigo_barras_empleados.php?opcion=$res[1]&nombre=$_POST[txt_nombre_baja]";
			}

			if($res[0]!=""){
			?>
			<input type="button" name="btn_fotosEmp" class="botones" value="Datos Empleados"
				onMouseOver="window.estatus='';return true" title="Ver Fotos Empleados"
				onclick="window.open('<?php echo $ruta; ?>', '_blank','top=100, left=100, width=1035, height=723, status=no, menubar=no, resizable=yes, scrollbars=yes, toolbar=no, location=no, directories=no')" />
			&nbsp;&nbsp;&nbsp;&nbsp;
			<input type="submit" value="Exportar a Excel" title="Exportar a Excel los datos de los Empleados" class="botones"
				name="sbt_exportar" id="sbt_exportar" onmouseover="window.status='';return true;" />
			&nbsp;&nbsp;&nbsp;&nbsp;
			<input type="button" name="btn_codigoBarras" class="botones" value="Ver Codigos de Barras" style="width: 160px;"
				onMouseOver="window.estatus='';return true" title="Ver Fotos Empleados"
				onclick="window.open('<?php echo $ruta_codBarras; ?>', '_blank','top=100, left=100, width=1035, height=723, status=no, menubar=no, resizable=yes, scrollbars=yes, toolbar=no, location=no, directories=no')" />
			&nbsp;&nbsp;&nbsp;&nbsp;
			<?php 
			}
			?>
