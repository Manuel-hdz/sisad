...
<title>Sistema de Gesti&oacute;n Empresarial, Producci&oacute;n y Operaci&oacute;n</title>
	...
	<script type="text/javascript" src="includes/ajax/regKardexPeriodo.js"></script>
	
    <style type="text/css">
		<!--				
		#titulo-consultar {position:absolute; left:30px; top:146px; width:173px; height:25px; z-index:11; }
		#consultar-kardex {position:absolute; left:30px; top:190px; width:620px; height:190px; z-index:15;}
		#consultar-kardex2 {position:absolute; left:30px; top:420px; width:620px; height:150px; z-index:12;}
		...
		#calendar-ini {position:absolute; left:475px; top:233px; width:30px; height:26px; z-index:18; }
		#calendar-fin {position:absolute; left:475px; top:268px; width:30px; height:26px; z-index:19; }
		-->
    </style>
</head>
<body>
	<div id="barra"> <img src="../../images/title-bar-bg.gif" width="999" height="30" /></div>
	<div class="titulo_barra" id="titulo-consultar">Consulta de Kardex</div><?php	

	//Si no esta definido ningun boton, mostrar el formulario de Base
	if (!isset($_POST["sbt_consultarArea"]) && !isset($_POST["sbt_consultarIndividual"])){?>
	<fieldset class="borde_seccion" id="consultar-kardex">
		<legend class="titulo_etiqueta">Consultar por &Aacute;rea y Fecha</legend>	
		<br>		
		<form name="frm_consultarKardex1" method="post" action="frm_consultarKardex.php" onsubmit="return valFormSeleccionarKardex1(this);">
			<table width="100%" border="0" cellpadding="5" cellspacing="5" class="tabla_frm">
				<tr>
				  <td width="70" ><div align="right">&Aacute;rea</div></td>
					<td width="130"><?php
							$conn = conecta("bd_recursos");		
							$stm_sql = "SELECT DISTINCT area FROM empleados WHERE id_empleados_empresa>0 ORDER BY area";
				...
				<tr>
					<td align="center" colspan="4">
						<input name="sbt_consultarArea" type="submit" class="botones" id="sbt_consultarArea" 
						...
						onclick="location.href='menu_kardex.php'" />
					</td>
                </tr>
            </table>    
		</form>    			 		
	...
	<fieldset id="consultar-kardex2" class="borde_seccion">
		<legend class="titulo_etiqueta">Consultar Kardex Individual</legend>	
		<br>
		<form name="frm_consultarKardex2" method="post" action="frm_consultarKardex.php" onsubmit="return valFormSeleccionarKardex2(this);">
	...
	<?php
		...
		if (isset($_POST["sbt_consultarArea"])){
			...
			?>
				<input name="sbt_exportarExcel" type="button" value="Exportar a Excel" class="botones" onclick="location.href='guardar_reporte.php?kardeDetalle=1&fechaI=<?php echo $_POST["txt_fechaIni"]?>&fechaF=<?php echo $_POST["txt_fechaFin"]?>&tipo=<?php echo $tipo?>&criterio=<?php echo $criterio?>&dias=<?php echo $dias?>'"/>
				&nbsp;&nbsp;&nbsp;&nbsp;
