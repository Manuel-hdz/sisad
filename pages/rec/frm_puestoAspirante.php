...
<head>
    ...
    <style type="text/css">
		<!--
		#titulo-puesto {position:absolute; left:30px; top:146px; width:447px; height:20px; z-index:11; }
		#tabla-registrarPuestoAspirante { position:absolute; left:20px; top:190px; width:949px; height:247px; z-index:12; padding:15px; padding-top:0px;}
		#calendario {position:absolute; left:822px; top:232px; width:30px; height:26px; z-index:13;}
		#resultados-PuestoAspirante { position:absolute; left:38px; top:485px; width:908px; height:159px; z-index:22; overflow:scroll; }
		-->
    </style>
</head>
<body><?php 	
	...
	if(isset($_SESSION['datosPuestoAspirante'])){?>
		<div id="resultados-PuestoAspirante" class='borde_seccion2' align="center"><?php
			mostrarPuestosAspirante();?>
		</div><?php
	}?>
	
	<div id="barra"><img src="../../images/title-bar-bg.gif" width="999" height="30" /></div>
    <div class="titulo_barra" id="titulo-puesto">Registrar el Puesto Recomendado para el Aspirante a Empleo</div>
	<fieldset class="borde_seccion" id="tabla-registrarPuestoAspirante">
	<legend class="titulo_etiqueta">Registrar Área y Puestos Recomendados para el Nuevo Aspirante</legend>	
	<br>
	<!--En  la propiedad action=""  de este formulario debera contener el nombre del formulario   -->
	<form onSubmit="return valFormRegistrarPuestoAspirante(this);" name="frm_puestoAspirante" method="post" action="frm_puestoAspirante.php" >
	<table width="100%" cellpadding="5" cellspacing="5" class="tabla_frm">
		...
		<tr>
			...
			<td><?php
				...
				if($datos = mysql_fetch_array($result)){?>
					<select name="cmb_area" id="cmb_area" size="1" class="combo_box" onchange="cargarCombo(this.value,'bd_recursos','area_puesto','puesto','area','cmb_puesto','Puesto Recomendado','');">
						...
						do{
							if ($datos['area'] == $cmb_area){
								echo "<option value='$datos[area]' selected='selected'>$datos[area]</option>";
							}
							else{
			...
			<td align="right">
			    ...
			    Agregar Nueva &Aacute;rea Recomendada </div></td>
		...
		<tr>
			...
			<td><select name="cmb_puesto" id="cmb_puesto" class="combo_box">
		...
		<tr>
			<td colspan="4">
				...
				<?php if(isset($_SESSION['datosPuestoAspirante'])){//Si al menos un puesto ha sido agregado, mostrar el boton de registrar Contactos ?>
					<input name="sbt_registrarContactosAspirante" type="submit"  class="botones_largos"  value="Registrar Contactos" title="Registrar los Datos de los Contactos del Aspirante" 
					onmouseover="window.status='';return true" onclick="hdn_botonSeleccionado.value='registrarContacto'" />
				<?php } ?>
				&nbsp;&nbsp;&nbsp;
				<input name="sbt_registrarPuesto" type="submit" class="botones_largos" id="sbt_puesto" title="Agregar Puesto Recomendado para el Aspirante Registrado" 
				...
				&nbsp;&nbsp;&nbsp;
				<input name="btn_cancelar" type="button" class="botones" value="Cancelar" title="Regresar a la Sección de Registrar Aspirante" 
