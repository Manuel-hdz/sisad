...
<body>
	...
	<br>
	<form name="frm_registrarNomina" method="post" action="frm_registrarNominaInternaMac.php" onsubmit="return valSeleccionarNomina(this);">
		<table width="415"  cellpadding="5" cellspacing="5" class="tabla_frm">
			<tr>
				...
				<td colspan="4"><?php
					...
					$result=mysql_query("SELECT DISTINCT id_nomina FROM nominas WHERE area = 'MANTENIMIENTO SUPERFICIE' AND finalizada = '1' ORDER BY id_nomina");
					if($nominas=mysql_fetch_array($result)){ ?>
						<select name="cmb_nomina" id="cmb_nomina" size="1" class="combo_box" onchange="cargarTxtFechas(this.value,'txt_fechaIni','txt_fechaFin','bd_mantenimiento')">
