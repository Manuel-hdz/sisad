<?php
	...
	if(isset($_GET['bono'])){
		...
		//Crear la Sentencia SQL
		$sql_stm = "SELECT DISTINCT T1. * , T2.descripcion AS centro, T3.descripcion AS cuenta
		...
		if($datos=mysql_fetch_array($rs)){
			...
			do{
				//Prevenir errores en el archivo XML generado, cuando el texto contenga
				//acentos o algun caracter especial
				echo utf8_encode("<id$cont>$datos[id_bono]</id$cont>");
				echo utf8_encode("<descripcion$cont>$datos[centro], $datos[cuenta]...Semana $datos[semana]</descripcion$cont>");
				$cont++;
			}while($datos=mysql_fetch_array($rs));
			echo "</existe>";
		}//Cierre if($datos=mysql_fetch_array($rs))
