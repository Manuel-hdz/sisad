<?php
	...
	**/
	...
	function mostrarTrabajadores(){
		//Creamos la sentencia SQL para mostrar los datos de todos los empleados que no correspondan a CONCRETO,DESARROLLO y MANTENIMIENTO
		$stm_sql="SELECT rfc_empleado,CONCAT(nombre,' ',ape_pat,' ',ape_mat) AS nombre,area,puesto FROM empleados WHERE area!='CONCRETO' AND area!='DESARROLLO' AND area!='MANTENIMIENTO' AND id_empleados_empresa>0 ORDER BY area,id_empleados_empresa";
			...
			do{
				...
				echo "					
						...
						<td class="<?php echo $nom_clase; ?>" align="center">
							<?php
								//cargarCombo("cmb_turno$cont","nom_turno","turnos","bd_recursos","Turno",$turno);
		...
		do{
			if (isset($_POST["ckb_seleccionar".$cont])){
				...
				if (!$rs){
					...
					registrarOperacion("bd_recursos","$rfc","ActualizarRol",$_SESSION['usr_reg']);
				}
			}
			$cont++;
		}while($cont<=$tam);
