<?php
	...
	**/
	...
	function mostrarDetalleCap($ckb){
		//Funcion que al cambiar de valor, indica que se encontraron resultados
		$band=0;
		...
		ON rfc_empleado = empleados_rfc_empleado AND capacitaciones_id_capacitacion= '$ckb') ORDER BY area";
		...
		if($datos=mysql_fetch_array($rs)){
			...
			<table cellpadding='5'>
				<tr>
					...
					<td class='nombres_columnas' align='center' colspan='2'>CONSTANCIAS</td>
				</tr>";
			$nom_clase = "renglon_gris";
			$cont = 1;	
			do{
				echo "<tr>";
						...
						<td class="nombres_filas" align="center">
							<input type="checkbox" name="ckb_selEmpleado<?php echo $cont?>" id="ckb_selEmpleado<?php echo $cont?>" value="<?php echo $datos['rfc_empleado'];?>"
							title="Borrar Asistencia a Capacitaci&oacute;n del Empleado Seleccionado"/>
						</td>
						<?php
				echo "		
						...
						<td class="<?php echo $nom_clase; ?>" align="center">
							<input type="button" name="btn_constancia" class="botones" value="Constancia DC-3" onMouseOver="window.estatus='';return true" 
                            title="Ver Constancia DC-3 del Empleado <?php echo $datos['nombre'];?>" onclick="window.open('../../includes/generadorPDF/capacitacion.php?id=<?php echo $datos['rfc_empleado']; ?>&cap=<?php echo $ckb?>', '_blank','top=100, left=100, width=1035, height=723, status=no, menubar=yes, resizable=yes, scrollbars=yes, toolbar=no, location=no, directories=no')"/>							
						</td>
						<td class="<?php echo $nom_clase; ?>" align="center">
							<input type="button" name="btn_constancia" class="botones" value="Constancia DC-4" onMouseOver="window.estatus='';return true" 
                            title="Ver Constancia DC-4 del Empleado <?php echo $datos['nombre'];?>" onclick="window.open('../../includes/generadorPDF/capacitacionDC4.php?id=<?php echo $datos['rfc_empleado']; ?>&cap=<?php echo $ckb?>', '_blank','top=100, left=100, width=1035, height=723, status=no, menubar=yes, resizable=yes, scrollbars=yes, toolbar=no, location=no, directories=no')"/>
						</td>
					</tr><?php
	...
	function eliminarAsistentes(){
		$band="0";
		//Recuperar el id de la capacitacion
		$idCapacitacion=$_POST["ckb"];
		//Conectar a la BD de Recursos
		$conn=conecta("bd_recursos");
		//Recorrer el POST buscando los checkbox
		foreach($_POST as $ind => $value){
			if(substr($ind,0,15)=="ckb_selEmpleado"){
				//Sentencia de borrado
				$sql="DELETE FROM empleados_reciben_capacitaciones WHERE empleados_rfc_empleado='$value' AND capacitaciones_id_capacitacion='$idCapacitacion'";
				$rs=mysql_query($sql);
				if(!$rs){
					$band="1";
					break;
		...
		if($band=="0"){
			...
			<script type="text/javascript" language="javascript">
				setTimeout("alert('Trabajador(es) Borrados Correctamente');",200);
			</script>
			<?php
		}
		else{
			?>
			<script type="text/javascript" language="javascript">
				setTimeout("alert('Se Generó un Error al Borrar a <?php echo $band;?>');",200);
			</script>
			<?php
		}
	}
