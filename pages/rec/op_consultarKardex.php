<?php
	...
	**/
	...
	function kardexArea(){
		...
		if($datos=mysql_fetch_array($rs)){
			...
			if($incidenciaFechas!=""){
				...
				<br /><br /><br />
				<p align="center">
				<table class='tabla_frm' cellpadding='5' width='75%' align="center">
				<caption class='titulo_etiqueta'>Registrar Incidencia para Todo el Periodo</caption>
				<tr><td colspan="4">&nbsp;</td></tr>
				<tr>
					<td align="right" valign="top"><div align="right">Estado</div></td>
					<td valign="top">
						<select name="cmb_estado" id="cmb_estado" class="combo_box">
							<option value="">Estado</option>
							<option value="A" title="Asistencia">ASISTENCIA</option>
							<option value="F" title="Falta">FALTA</option>
							<option value="d" title="Descanso">DESCANSO</option>
							<option value="V" title="Vacaciones">VACACIONES</option>
							<option value="r" title="Retardo">RETARDO</option>
							<option value="F/J" title="Falta/Justificada">FALTA JUSTIFICADA</option>
							<option value="P" title="Permiso Sin Goce de Sueldo">PERMISO SIN GOCE</option>
							<option value="P/G" title="Permiso Con Goce de Sueldo">PERMISO CON GOCE</option>
							<option value="E" title="Incapacidad por Enfermedad General">INCAPACIDAD ENFERMEDAD GENERAL</option>
							<option value="RT" title="Incapacidad por Accidente de Trabajo">INCAPACIDAD ACCIDENTE TRABAJO</option>
							<option value="T" title="Incapacidad en Trayecto">INCAPACIDAD EN TRAYECTO</option>
							<option value="D" title="Sanci&oacute;n Discplinaria">SANCION DISCIPLINARIA</option>
							<option value="R" title="Regresaron">REGRESARON</option>
						</select>
					</td>
					...
					<td valign="top">
						...
						rows="3" cols="30" onkeypress="return permite(event,'num_car', 0);"></textarea>
					</td>
				</tr>
				<tr>
					<td colspan="4" align="center">
					<input type="button" name="btn_guardar" id="btn_guardar" title="Guardar la Incidencia para todo el Periodo" class="botones"
					value="Guardar" onclick="guardarIncPeriodo('<?php echo $incidenciaFechas?>',cmb_estado.value,txa_comentarios.value,'<?php echo $fechaIMod?>','<?php echo $fechaFMod?>');"/>
					</td>
				</tr>
				</table>
				</p>
	...
	function obtenerChecada($fecha,$rfc,$ctrl){
		...
		$checada="<input type='text' name='ckb_$fecha $ctrl' id='ckb_$fecha $ctrl' class='caja_de_num' size='1' readonly='readonly' onclick='asignarEstadoKardexArea(this);' value='$estado' style='font-size:19px;cursor:pointer;$color' title='Asignar la Incidencia del $fechaMostrar para $nombre'/>";
		return $checada;
	}
?>