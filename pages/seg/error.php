...
<body>
	<div id="parrilla-menu1" align="center">
		...
		if(isset($_GET['err'])) { 
			if($err=="AccesoNegado") {
				$usuarios=verificarUsuarioPermisos("Seguridad");
				$band=0;
				if(count($usuarios) != 0){
				...
				//si el usuario no es de Seguridad, buscarlo en SeguridadAmbiental
				if($band==0){
					$usuarios=verificarUsuarioPermisos("SeguridadAmbiental");
					$band2=0;
					foreach($usuarios as $ind=> $valor){
						if($usr_reg==$valor){
							$band2=1;
            	...
            	<meta http-equiv="refresh" content="7;url=inicio_seguridad.php">
				<p>
