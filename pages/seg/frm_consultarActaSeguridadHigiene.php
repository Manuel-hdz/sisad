...
<body>
    ...
    <legend class="titulo_etiqueta">Consultar Actas de Seguridad por Fecha </legend>
    <form  onsubmit="return valFormConsASH(this);"name="frm_consultarActa" id="frm_consultarActa" method="post" action="frm_consultarActaSeguridadHigiene2.php">
      <br />
      <table width="349" height="127"  cellpadding="5" cellspacing="5" class="tabla_frm">
        <tr>
          <td><div align="right">*Fecha Inicio </div></td>
          <td width="219"><input name="txt_fechaIni" type="text" id="txt_fechaIni" size="10" maxlength="15" value="<?php echo date("d/m/Y", strtotime("-30 day"));?>" 
        ...
        <tr>
          <td height="45" colspan="9"><div align="center">
              <input name="sbt_consultar" type="submit" class="botones" id="sbt_consultar" value="Consultar" title="Consultar Acta Seguridad Higiene"
					onmouseover="window.status='';return true"/>
            &nbsp;&nbsp;&nbsp;
            <input name="btn_regresar" type="button" class="botones"  value="Regresar" title="Regresar al Men&uacute; Seguridad e Higiene" 
			onmouseover="window.status='';return true" 	onclick="location.href='menu_actaSeguridadHigiene.php'"/>
          </div></td>
        </tr>
      </table>
    </form>
    ...
    <legend class="titulo_etiqueta">Consultar Actas de Seguridad por Clave </legend>
    <form  onsubmit="return valFormASHClave(this);"name="frm_consultarActa2" id="frm_consultarActa2" method="post" action="frm_consultarActaSeguridadHigiene2.php">
      <br />
      <table width="349" height="127"  cellpadding="5" cellspacing="5" class="tabla_frm">
        <tr>
          ...
          <td width="219"><?php  
			...
			$result=mysql_query("SELECT DISTINCT id_acta_comision FROM acta_comision ORDER BY id_acta_comision");
			if($id=mysql_fetch_array($result)){?>
              <select name="cmb_id" id="cmb_id" size="1" class="combo_box">
                <option value="">CLAVE ACTA</option>
                <?php 
				  do{
						if ($id['id_acta_comision'] == $cmb_id){
							echo "<option value='$id[id_acta_comision]' selected='selected'>$id[id_acta_comision]</option>";
						}
						else{
							echo "<option value='$id[id_acta_comision]'>$id[id_acta_comision]</option>";
						}
              ...
              <?php }
					...
					echo "<label class='msje_correcto'> No Existen Actas Registradas</label>";?>
					<input type='hidden' name='cmb_id' id='cmb_id' value=''/>
              <?php }?>
          </td>
        </tr>
        <tr>
