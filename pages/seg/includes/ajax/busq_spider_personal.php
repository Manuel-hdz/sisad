	...
	if(!$db) {
		...
		if(isset($_POST['queryString'])) {
			...
			if(strlen($queryString) >0) {
				...
				$query = $db->query("SELECT rfc_empleado,puesto,CONCAT(nombre,' ',ape_pat,' ',ape_mat) AS nombre FROM empleados WHERE CONCAT(nombre,' ',ape_pat,' ',ape_mat) LIKE '%$queryString%' AND estado_actual = 'ALTA' LIMIT 10");
				if($query) {
					...
					while($result = $query ->fetch_object()){// Establece los resultados a modo de lista.
						...
						echo utf8_encode('
								...
								obtenerAntiguedadEmp(\''.$result->nombre.'\');">'.$result->nombre.'</li>
							');
