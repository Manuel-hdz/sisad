...
function lookup(cajaTexto,nomTabla,depto,num) {
	...
	if(inputString.length == 0) {
		...
		$.post("includes/ajax/busq_spider_personalRec.php?nomCajaTexto="+nomCajaTexto+"&nomTabla="+nomTabla+"&depto="+depto+"&num="+num, {queryString: ""+inputString+""}, function(data){
...
/*Esta función se utiliza para desplegar el nombre de los empleados registrados en recursos humanos y al mismo tiempo obtener el RFC del empleado seleccionado a traves
...
function obtenerNombreRFCEmpleado(cajaTexto,nomTabla,depto,num) {
	...
	if(inputString.length == 0) {
		...
		var nomCajaTexto = cajaTexto.name;
		//Enviar la petición al servidor para realizar la consulta de los datos que serán mostrados en el Layer de sugerencias
		$.post("includes/ajax/busq_spider_personalRec.php?nomCajaTexto="+nomCajaTexto+"&nomTabla="+nomTabla+"&depto="+depto+"&num="+num+"&ctrlOnclik=1",
...
/*Esta función se utiliza para desplegar el nombre de los empleados registrados en recursos humanos y al mismo tiempo validar que el empleado tenga más de 3 meses de antiguedad
...
function obtenerEmpleadoValidarEstado(cajaTexto,nomTabla,depto,num) {
	...
	if(inputString.length == 0) {
		...
		var nomCajaTexto = cajaTexto.name;
		//Enviar la petición al servidor para realizar la consulta de los datos que serán mostrados en el Layer de sugerencias
		$.post("includes/ajax/busq_spider_personalRec.php?nomCajaTexto="+nomCajaTexto+"&nomTabla="+nomTabla+"&depto="+depto+"&num="+num+"&ctrlOnclik=2",
			   {queryString: ""+inputString+""}, 
			   function(data){
					//Si los datos obtenidos son mayores que 0, entonces desplegar el Layer con las sugerencias
					if(data.length >0) {
