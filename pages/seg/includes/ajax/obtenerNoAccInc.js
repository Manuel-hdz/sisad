/**
  ...
  */
	...
	function obtenerNoAccInc(combo){
		if(combo!=""){
			//Crear la URL, la cual será solicitada al Servidor 
			var url = "includes/ajax/obtenerNoAccInc.php?combo="+combo;
			...
			*variará al menos en el valor de uno de los parámetros, el navegador estará obligado siempre a realizar la petición directamente al 
			...
			cargarNoAccInc(url, "GET", procesarAccInc);
		}
		else{
			document.getElementById("txt_noAcc").value="";	
			document.getElementById("label_tipo").innerHTML="";	
		}
	...
	function cargarNoAccInc(url, metodo, funcion) {
 		...
 		if(peticionHTTP.readyState==READY_STATE_COMPLETE){
			if(peticionHTTP.status==200){
				...
				if(existe=="true"){
					...
					document.getElementById("txt_noAcc").value=cantidad;
					if(tipo=="INCIDENTE"){
						document.getElementById("label_tipo").innerHTML="Inc";	
					}
					else{
						document.getElementById("label_tipo").innerHTML="Acc";	
					}
				}																
