<?php
	/**
	  * Nombre del Módulo: Aseguramiento Calidad- Seguridad Industrial                                               
	...
	function mostrarRecordatorios(){
		...
		if(isset($_GET['seg'])){
			...
			//Almacenamos el valor del get
			$get = "marco_descargaSeg.php";
			...
			do{
				...
				<?php if($noReg!=0){
						if($noReg==1){?>
							...
							onClick="javascript:window.open('<?php echo $get; ?>?id_documento=<?php echo $arrArch['id_documento'];?>&nomArchivo=<?php echo $arrArch['nom_archivo'];?>&ruta=<?php echo $arrArch['ruta'];?>&nombre=<?php echo $arrArch['nombre'];?>&tipo=<?php echo $arrArch['tipo_archivo'];?>',
				 ...
				 <?php }
					   elseif($noReg>1){?>
							...
							title="Descargar Documento<?php echo $arrArch['nombre'];?>" 
							onClick="javascript:window.open('verArchivosExternos.php?id_alerta=<?php echo $datos['id_alerta'];?>&<?php echo $depto;?>=1',
	...
	function mostrarArchivos($idAlerta, $bd, $marcoDescarga){
		...
		//Realizar la conexion a la BD que corresponda
		$conn = conecta($bd);
			...
			do{			
				...
				<tr>
					...
					<td class="<?php echo $nom_clase; ?>" align="center">
						...
						onClick="javascript:window.open('<?php echo $marcoDescarga;?>?id_documento=<?php echo $datos['id_documento'];?>&nomArchivo=<?php echo $datos['nom_archivo'];?>&ruta=<?php echo $datos['ruta'];?>&nombre=<?php echo $datos['nombre'];?>&tipo=<?php echo $datos['tipo_archivo'];?>',
						'_blank','top=300, left=450, width=1, height=1, status=no, menubar=no, resizable=no, scrollbars=yes, toolbar=no, location=no, directories=no');"/>		
				</td><?php 
			echo "</tr>";
				//Determinar el color del siguiente renglon a dibujar
