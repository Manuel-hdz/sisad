<?php
	...
	function obtenerIdRequisicion(){
		...
		$stm_sql = "SELECT COUNT(id_requisicion) AS cant FROM requisiciones WHERE id_requisicion LIKE 'SEG$mes$anio%'";
	...
	function guardarRequisicion($txa_justificacionReq,$hdn_fecha,$txt_areaSolicitante,$txt_solicitanteReq,$txt_elaboradorReq){
		...
		if($rs){			
			...
			if($band==0){
				...
				registrarOperacion("bd_seguridad",$_SESSION['id_requisicion'],"GenerarRequisicion",$_SESSION['usr_reg']);
	...
	function subirFotos($clave, $nombre){
		...
		if (is_uploaded_file($_FILES['file_documento']['tmp_name'])) { 
			...
			if (!file_exists($carpeta."/".$nombreMat)){
				...
				?>
				<script>
				setTimeout("alert('Imagen Cargada Correctamente'); window.close();",500);
				</script>
				<?php
			}
			else{?>
			<script language="javascript" type="text/javascript">
				if(confirm("¿El Registro ya Cuenta con Una Imagen Cargada\nDesea Sustituir la Imagen?")){<?php 
					...
					redimensionarFoto($carpeta."/".$nombreMat,$nombreMat,$carpeta."/",100,100);
				?>
					setTimeout("alert('Imagen Sustituida Correctamente'); window.close();",500);
				}
				else{
					window.close();
				}
	...
	function borrarFotosSesionSeguridad(){
		//Almacenamos el valor de la clave de la requisicion
		$carpeta=$_SESSION["fotosReq"][0]["clave"];
		//Borrar los ficheros temporales
		$h=opendir('seg/documentos/'.$carpeta);
		while ($file=readdir($h)){
			if (substr($file,-4)=='.jpg'){
				@unlink("seg/documentos/".$carpeta."/".$file);
			}
		}
	}//Fin de la funcion borrarFotosSesionAseguramiento()
