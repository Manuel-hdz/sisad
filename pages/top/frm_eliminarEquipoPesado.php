...
<?php
	...
	if(!verificarPermiso($usr_reg,$_SERVER['PHP_SELF'])){	
		...
		include("op_eliminarEquipoPesado.php");
...
<body>	
	...
	<div class="titulo_barra" id="titulo-modificar">Seleccionar el Nombre de Obra con Equipo</div>
	
	<?php
	if(isset($_POST["sbt_eliminar"]))
		eliminarRegistro($_POST["rdb_idRegistro"]);
	
	if(!isset($_POST["sbt_seleccionarObra"])){
	?>
		<fieldset class="borde_seccion" id="tabla-seleccionarObra" name="tabla-seleccionarObra">
		<legend class="titulo_etiqueta">Seleccionar la Obra de Equipo a Eliminar</legend>	
		<br>
		<form onSubmit="return valFormSeleccionarObraEq(this);" name="frm_modificarObraEqP" method="post" action="frm_eliminarEquipoPesado.php">
	...
	<?php
		...
		?>
		<form onSubmit="return valFormElimBitEqP(this);" name="frm_eliminarEquipoPesado" method="post" action="frm_eliminarEquipoPesado.php">
				...
				<tr>
					<td>
						...
						if($res==1){?>
							...
							onmouseover="window.status='';return true" title="Eliminar Registro Seleccionado" />&nbsp;&nbsp;&nbsp;&nbsp;
						<?php
						}?>               
						<input name="btn_regresar2" type="button" class="botones" value="Regresar" onclick="location.href='frm_eliminarEquipoPesado.php';"
