...
//	else{
		...
		include("op_registrarBitAnclas.php");
	...
	<script type="text/javascript" language="javascript">
		function obtenerTotalPesos(cantidad){
			//Precio Unitario en Pesos
			var precio=document.getElementById("txt_precioUMN").value.replace(/,/g,'');
			//Cantidad Total
			var total=document.getElementById("txt_cantidadTotal").value.replace(/,/g,'');
			//Obtener el Total en Pesos
			var precioTotal=(precio*total);
			//Asignar el precio total en Pesos
			formatCurrency(precioTotal,"txt_totalMN");
			//Obtener el Importe
			var importe=(precioTotal*1)+(document.getElementById("txt_totalUSD").value.replace(/,/g,'')*1);
			//Asignar el importe
			formatCurrency(importe,"txt_totalImporte");
			//Calcular el total en Dolares pasando el total en Dolares como parametro
			obtenerTotalUSD(document.getElementById("txt_tasaCambio").value.replace(/,/g,''));
		}
	...
	<style type="text/css">
		...
		#tabla-registrarAnclas {position:absolute;left:30px;top:190px;width:720px;height:330px;z-index:12;}
...
<body>	
	...
	<div class="titulo_barra" id="titulo-modificar">Registrar Anclas</div>
	
	<?php
	
	if(isset($_POST["sbt_guardarReg"]))
		guardarRegistro();
	else{	
		$tipoObra=$_GET["tipo"];
		$concepto=obtenerDato("bd_topografia","obras","nombre_obra","id_obra",$_GET["id"]);
		$pumn=number_format(obtenerDato("bd_topografia","obras","pumn_estimacion","id_obra",$_GET["id"]),2,".",",");
		$puusd=number_format(obtenerDato("bd_topografia","obras","puusd_estimacion","id_obra",$_GET["id"]),2,".",",");

		$tCambio=obtenerdato("bd_topografia","tasa_cambio","t_cambio","id",1);
		?>
		<fieldset class="borde_seccion" id="tabla-registrarAnclas" name="tabla-registrarAnclas">
		<legend class="titulo_etiqueta">Complementar los Datos</legend>	
		<br>
		<form name="frm_guardarBitAnclas" method="post" action="frm_registrarAnclas.php" onsubmit="">
		<input type="hidden" name="hdn_idObra" id="hdn_idObra" value="<?php echo $_GET["id"]?>"/>
		<table cellpadding="5" cellspacing="5" class="tabla_frm" width="100%">
			...
			<tr>
				...
				<td colspan="3">
					<input type="text" name="txt_cantidadTotal" id="txt_cantidadTotal" class="caja_de_texto" value="" size="10" onkeypress="return permite(event,'num',2);" onchange="obtenerTotalPesos(this.value)"/>
				</td>
			</tr>
			<tr>
			...
			<tr>
				<td colspan="4" align="center">
					Total M.N.
					$<input type="text" name="txt_totalMN" id="txt_totalMN" class="caja_de_texto" value="0.00" readonly="readonly"/>
					&nbsp;&nbsp;&nbsp;&nbsp;
					Total USD
					$<input type="text" name="txt_totalUSD" id="txt_totalUSD" class="caja_de_texto" value="0.00" readonly="readonly"/>
					&nbsp;&nbsp;&nbsp;&nbsp;
					Importe Total
					$<input type="text" name="txt_totalImporte" id="txt_totalImporte" class="caja_de_texto" value="0.00" 
			...
			<tr>
				<td colspan="4" align="center">
					<input type="submit" name="sbt_guardarReg" id="sbt_guardarReg" value="Guardar" class="botones" title="Guardar Registro de Anclas" 
		...
		<div id="calendarioElaboracion">
			...
			onclick="displayCalendar(document.frm_guardarBitAnclas.txt_fechaRegistro,'dd/mm/yyyy',this)" 
			onmouseover="window.status='';return true"width="25" height="25" border="0" align="absbottom" 
			title="Seleccionar la Fecha del Registro Anclas"/> 
		</div>
	<?php
	}
	?>
