	...
	function consultarSolicitud(){
		...
		if($datos=mysql_fetch_array($rs)){
			echo "				
				...
				?>
			<div id='tabla-exaMedico2'>
				<input name="sbt_registrarEmpleadoExt" type="button" class="botones_largos" id="sbt_registrarEmpleadoExt"  value="Realizar Historial Clinico" 
				title="Consultar la Solicitud de Ex&aacute;men M&eacute;dico" 
				onclick="enviarDatosHC(frm_enviarDatosHCExterno.hdn_empresa.value);" />
			</div><?php 
			$nom_clase = "renglon_gris";			
			$cont = 1;
							...
							echo "
								<tr>
									<td class='$nom_clase' align='center'>									
										<input type='radio' name='rdb_empExt' value='$numSolicitud"."_"."$datosEmpExt[id_registro]'/>
									</td>
