<?php
	...
	function obtenerIdRequisicion(){
		...
		$stm_sql = "SELECT COUNT(id_requisicion) AS cant FROM requisiciones WHERE id_requisicion LIKE 'USO$mes$anio%'";
	...
	function guardarRequisicion($txa_justificacionReq,$hdn_fecha,$txt_areaSolicitante,$txt_solicitanteReq,$txt_elaboradorReq){
		...
		if($rs){			
			...
			if($band==0){
				...
				registrarOperacion("bd_clinica",$_SESSION['id_requisicion'],"GenerarRequisicion",$_SESSION['usr_reg']);
	...
	function borrarFotosSesionUSO(){
		//Almacenamos el valor de la clave de la requisicion
		$carpeta=$_SESSION["fotosReq"][0]["clave"];
		//Borrar los ficheros temporales
		$h=opendir('uso/documentos/'.$carpeta);
		while ($file=readdir($h)){
			if (substr($file,-4)=='.jpg'){
				@unlink("uso/documentos/".$carpeta."/".$file);
			}
		}
