...
<head>
	...
	<style type="text/css">
		...
		<br />
		<form name="frm_registrarMedicamento" method="post" onsubmit="return valFormSelMedicamentos(this);" action="verRegMedicamento.php?btn=<?php echo $_GET["btn"]?>">
			...
			if(isset($_SESSION["medicamento"])){
				...
				if(isset($_SESSION["medicamento"])){
					...
					echo "
						...
						<caption class='titulo_etiqueta'>Medicamentos</caption>
							<tr>
								...
								<th class='nombres_columnas' align='center'>CANTIDAD ORIGINAL</th>
								<th class='nombres_columnas' align='center'>CANTIDAD SALIDA</th>
								<th class='nombres_columnas' align='center'>CANTIDAD TOTAL</th>
					...
					foreach($_SESSION["medicamento"] as $ind=>$value){
						//Sentencia SQL
						$sql_stm="SELECT codigo_med,nombre_med,existencia_actual,unidad_despacho FROM catalogo_medicamento WHERE id_med='$ind'";
						$rs=mysql_query($sql_stm);
						if($datos=mysql_fetch_array($rs)){
							$resto=$datos["existencia_actual"]-$value;
							echo " 
							<tr>
								<td class='$nom_clase' align='center'>$datos[codigo_med]</td>
								<td class='$nom_clase' align='center'>$datos[nombre_med]</td>
								<td class='$nom_clase' align='center'>$datos[existencia_actual]</td>
								<td class='$nom_clase' align='center'>$value</td>
								<td class='$nom_clase' align='center'>$resto $datos[unidad_despacho](S)</td>
								<td class='$nom_clase' align='center'>
									<input type=\"image\" src=\"../../images/borrar.png\" width=\"30\" height=\"25\" onclick=\"document.getElementById('hdn_cerrar').value='0';location.href='verRegMedicamento.php?btn=$boton&idMed=$ind'\"/>
								</td>
							</tr>";
